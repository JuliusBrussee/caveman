...
$Flag = Join-Path $ClaudeDir ".layman-active"
...
$Valid = @('off','summary','explain','brief','lite','full','ultra','wenyan-lite','wenyan','wenyan-full','wenyan-ultra','commit','review','compress')
if (-not ($Valid -contains $Mode)) { exit 0 }

$Esc = [char]27
if ([string]::IsNullOrEmpty($Mode) -or $Mode -eq "summary") {
    [Console]::Write("${Esc}[38;5;172m[LAYMAN]${Esc}[0m")
} else {
    $Suffix = $Mode.ToUpperInvariant()
    [Console]::Write("${Esc}[38;5;172m[LAYMAN:$Suffix]${Esc}[0m")
}
