#!/usr/bin/env node
// layman — shared configuration resolver
//
// Resolution order for default mode:
//   1. LAYMAN_DEFAULT_MODE environment variable
//   2. Config file defaultMode field:
//      - $XDG_CONFIG_HOME/layman/config.json (any platform, if set)
//      - ~/.config/layman/config.json (macOS / Linux fallback)
//      - %APPDATA%\layman\config.json (Windows fallback)
//   3. 'summary'
...
function getConfigDir() {
  if (process.env.XDG_CONFIG_HOME) {
    return path.join(process.env.XDG_CONFIG_HOME, 'layman');
  }
  ...
  return path.join(os.homedir(), '.config', 'layman');
}
...
function getDefaultMode() {
  ...
  const envMode = process.env.LAYMAN_DEFAULT_MODE;
...
// flag path (~/.claude/.layman-active) with a symlink to clobber other files.
//
...
function safeWriteFlag(flagPath, content) {
  try {
    ...
    const tempPath = path.join(flagDir, `.layman-active.${process.pid}.${Date.now()}`);
