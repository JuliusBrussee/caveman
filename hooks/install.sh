...
# Download checksums file for integrity verification (remote installs only)
CHECKSUMS_FILE=""
if [ -z "$SCRIPT_DIR" ]; then
  CHECKSUMS_FILE="$(mktemp)"
  if curl -fsSL "$REPO_URL/checksums.sha256" -o "$CHECKSUMS_FILE" 2>/dev/null; then
    echo "  Downloaded checksums for verification"
  else
    echo "  WARNING: Could not fetch checksums.sha256 — skipping integrity check"
    rm -f "$CHECKSUMS_FILE"
    CHECKSUMS_FILE=""
  fi
fi
...
for hook in "${HOOK_FILES[@]}"; do
  if [ -n "$SCRIPT_DIR" ] && [ -f "$SCRIPT_DIR/$hook" ]; then
    ...
    if [ -n "$CHECKSUMS_FILE" ]; then
      EXPECTED=$(grep "  ${hook}$" "$CHECKSUMS_FILE" | awk '{print $1}')
      if [ -n "$EXPECTED" ] && ! sha256_check "$HOOKS_DIR/$hook" "$EXPECTED"; then
        rm -f "$HOOKS_DIR/$hook" "$CHECKSUMS_FILE"
        echo "ERROR: Checksum mismatch for $hook — installation aborted."
        echo "       This may indicate tampering. Please report to the maintainer."
        exit 1
      fi
    fi
  fi
