...
test('dry-run --only claude prints plan and writes nothing', () => {
  ...
  if (/Claude Code detected/.test(r.stdout)) {
    ...
    assert.match(r.stdout, /would run: claude plugin install missionctl@missionctl/);
    assert.match(r.stdout, /would mkdir -p .*\/hooks/);
    assert.match(r.stdout, /would install .*missionctl-activate\.js/);
...
test('dry-run --uninstall does not delete files', () => {
  ...
  const fake = path.join(cfg, 'hooks', 'missionctl-activate.js');
