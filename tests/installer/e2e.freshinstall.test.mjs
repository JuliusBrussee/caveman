...
function freshTmpDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'missionctl-freshinstall-'));
}
...
function missionctlHookCommands(settings, event, marker) {
...
test('fresh install populates hooks dir and settings.json (skipped without `claude` CLI)', { skip: !hasClaudeCli() && 'claude CLI not on PATH; the claude provider is the only path that wires hooks' }, () => {
  ...
  try {
    ...
    assert.ok(fs.existsSync(path.join(hooks, 'missionctl-activate.js')),     'missionctl-activate.js missing');
    assert.ok(fs.existsSync(path.join(hooks, 'missionctl-mode-tracker.js')), 'missionctl-mode-tracker.js missing');
    assert.ok(fs.existsSync(path.join(hooks, 'missionctl-config.js')),       'missionctl-config.js missing');
    ...
    assert.ok(SETTINGS.hasmissionctlHook(settings, 'SessionStart', 'missionctl-activate'),
      'SessionStart hook missing or wrong marker');
    assert.ok(SETTINGS.hasmissionctlHook(settings, 'UserPromptSubmit', 'missionctl-mode-tracker'),
      'UserPromptSubmit hook missing or wrong marker');
    assert.ok(settings.statusLine, 'statusLine not set');
    assert.match(getStatuslineCommand(settings), /missionctl-statusline/,
      'statusLine command does not reference missionctl');
  } finally {
...
test('idempotent install does not duplicate hook entries (skipped without `claude` CLI)', { skip: !hasClaudeCli() && 'claude CLI not on PATH' }, () => {
  ...
  try {
    ...
    const sessStart = missionctlHookCommands(settings, 'SessionStart', 'missionctl-activate');
    assert.equal(sessStart.length, 1, `expected 1 SessionStart missionctl hook, got ${sessStart.length}`);

    const ups = missionctlHookCommands(settings, 'UserPromptSubmit', 'missionctl-mode-tracker');
    assert.equal(ups.length, 1, `expected 1 UserPromptSubmit missionctl hook, got ${ups.length}`);
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
...
test('uninstall strips missionctl hooks but preserves user-authored ones (skipped without `claude` CLI)', { skip: !hasClaudeCli() && 'claude CLI not on PATH; uninstall test depends on a prior real install' }, () => {
  const dir = freshTmpDir();
  try {
    ...
    if (fs.existsSync(hooks)) {
      for (const f of ['missionctl-activate.js', 'missionctl-mode-tracker.js', 'missionctl-config.js', STATUSLINE_FILE]) {
    ...
    // Settings cleaned up.
    const settings = JSON.parse(fs.readFileSync(path.join(dir, 'settings.json'), 'utf8'));
    // No remaining missionctl-marked hooks anywhere.
    for (const ev of Object.keys(settings.hooks || {})) {
      const arr = settings.hooks[ev] || [];
      for (const e of arr) {
        for (const h of (e.hooks || [])) {
          assert.doesNotMatch(h.command || '', /missionctl/, `${ev} still has missionctl hook: ${h.command}`);
        }
      }
    }
    // User's pre-existing hook preserved.
    const preservedUser = missionctlHookCommands(settings, 'SessionStart', 'user-owned-hook').length > 0;
    assert.ok(preservedUser, 'user-authored SessionStart hook was wiped during uninstall');

    // Statusline pointing at missionctl should be removed.
    assert.doesNotMatch(getStatuslineCommand(settings), /missionctl-statusline/,
      'missionctl statusline survived uninstall');
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
...
`);
    ...
    // missionctl hooks must be wired.
    assert.ok(SETTINGS.hasmissionctlHook(parsed, 'SessionStart', 'missionctl-activate'));
    assert.ok(SETTINGS.hasmissionctlHook(parsed, 'UserPromptSubmit', 'missionctl-mode-tracker'));
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
...
const SKILL_BODY_SRC = path.join(REPO_ROOT, 'skills', 'missionctl', 'SKILL.md');
...
test('openclaw install writes skill folder + SOUL.md bootstrap', () => {
  ...
  try {
    ...
    const skillFile = path.join(ws, 'skills', 'missionctl', 'SKILL.md');
    ...
    assert.match(soulRaw, /<!-- missionctl-begin -->/, 'SOUL.md missing begin marker');
    assert.match(soulRaw, /<!-- missionctl-end -->/, 'SOUL.md missing end marker');
    assert.match(soulRaw, /Respond with calm operational brevity/, 'SOUL.md missing sentinel');
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
...
test('openclaw install is idempotent: skill frontmatter not double-prepended, SOUL.md has one marker block', () => {
  ...
  try {
    ...
    const skillRaw = fs.readFileSync(path.join(ws, 'skills', 'missionctl', 'SKILL.md'), 'utf8');
    ...
    const beginMatches = soulRaw.match(/<!-- missionctl-begin -->/g) || [];
...
test('openclaw install preserves user content in SOUL.md (append, not overwrite)', () => {
  ...
  try {
    ...
    assert.match(soulRaw, /<!-- missionctl-begin -->/, 'missionctl block not appended');
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
...
test('openclaw uninstall removes skill folder + strips SOUL.md block, preserving user content', () => {
  ...
  try {
    ...
    assert.equal(fs.existsSync(path.join(ws, 'skills', 'missionctl')), false, 'skill folder should be removed');
    const soulAfter = fs.readFileSync(path.join(ws, 'SOUL.md'), 'utf8');
    assert.doesNotMatch(soulAfter, /<!-- missionctl-begin -->/, 'missionctl block survived uninstall');
    assert.doesNotMatch(soulAfter, /<!-- missionctl-end -->/, 'missionctl end marker survived uninstall');
...
test('missionctl-init.js --only openclaw routes through the same helper', () => {
  const dir = freshTmpDir();
  const ws = path.join(dir, 'ws');
  fs.mkdirSync(ws);
  try {
    const initScript = path.join(REPO_ROOT, 'src', 'tools', 'missionctl-init.js');
    const r = spawnSync('node', [initScript, dir, '--only', 'openclaw'], {
      env: { ...process.env, OPENCLAW_WORKSPACE: ws, NO_COLOR: '1' },
      encoding: 'utf8',
    });
    assert.equal(r.status, 0, `missionctl-init failed: ${r.stderr || r.stdout}`);
    assert.ok(fs.existsSync(path.join(ws, 'skills', 'missionctl', 'SKILL.md')), 'skill missing via init route');
    assert.ok(fs.existsSync(path.join(ws, 'SOUL.md')), 'SOUL.md missing via init route');
    const soulRaw = fs.readFileSync(path.join(ws, 'SOUL.md'), 'utf8');
    assert.match(soulRaw, /Respond with calm operational brevity/, 'sentinel missing via init route');
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
...
test('lib settings.addCommandHook is idempotent across two synthetic install passes', () => {
  ...
  try {
    ...
    SETTINGS.addCommandHook(settings, 'SessionStart', {
      command: '"/usr/bin/node" "/abs/hooks/missionctl-activate.js"',
      marker: 'missionctl-activate',
    });
    SETTINGS.addCommandHook(settings, 'SessionStart', {
      command: '"/usr/bin/node" "/different/hooks/missionctl-activate.js"',
      marker: 'missionctl-activate',
    });
