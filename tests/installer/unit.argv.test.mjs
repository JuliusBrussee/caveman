...
test('--list prints provider matrix', () => {
  const r = run('--list');
  assert.equal(r.status, 0);
  assert.match(r.stdout, /missionctl provider matrix/);
...
test('--only with unknown agent id exits 2', () => {
  ...
  assert.match(r.stderr, /missionctl --list/);
});
...
test('--config-dir expands ~ to home directory', async () => {
  ...
  // match even when the missionctl plugin is already installed.
...
test('bare --with-mcp-shrink (no upstream) exits 2 with hint', () => {
  ...
  // that crashed on every Claude Code startup. missionctl-shrink is a proxy and
...
test('--with-mcp-shrink="<cmd>" registers wrapping that upstream', () => {
  ...
  if (/would run: claude mcp add missionctl-shrink/.test(r.stdout)) {
    assert.match(r.stdout, /claude mcp add missionctl-shrink .* npx -y missionctl-shrink npx @modelcontextprotocol\/server-filesystem \/tmp/);
  }
});
...
test('--with-mcp-shrink "<cmd>" (space-separated) also accepted', () => {
  ...
  assert.equal(r.status, 0);
  if (/would run: claude mcp add missionctl-shrink/.test(r.stdout)) {
    assert.match(r.stdout, /missionctl-shrink npx @modelcontextprotocol\/server-filesystem \/tmp/);
  }
});
...
test('--all does NOT auto-enable mcp-shrink (no sensible default upstream)', () => {
  ...
  assert.doesNotMatch(r.stdout, /wiring missionctl-shrink MCP proxy/);
});
