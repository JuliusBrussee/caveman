...
def verify_hook_install_flow() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="caveman-verify-") as temp_root:
        ...
        ensure("level: full" in activate.stdout, "activation output missing default level")
        ...
        ensure("level: ultra" in activate_custom.stdout, "activation output missing custom level")
        ...
        reinforcement = json.loads(ultra_prompt.stdout)
            ...
            "CAVEMAN MODE ACTIVE (ultra)" in reinforcement["hookSpecificOutput"]["additionalContext"],
