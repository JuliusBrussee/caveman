...
//   env MISSIONCTL_DEFAULT_MODE → repo-local (.missionctl/config.json or .missionctl.json,
...
// own ~/.config/missionctl/config.json.
const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'missionctl-userhome-'));
process.env.XDG_CONFIG_HOME = tmpHome;
delete process.env.MISSIONCTL_DEFAULT_MODE;

const { getDefaultMode, findRepoConfigPath } = require('../src/hooks/missionctl-config');

let passed = 0;
let failed = 0;

function test(name, fn) {
  const tmpBase = fs.mkdtempSync(path.join(os.tmpdir(), 'missionctl-repocfg-'));
  const origCwd = process.cwd();
  const origEnv = process.env.MISSIONCTL_DEFAULT_MODE;
  try {
    fn(tmpBase);
    passed++;
    console.log(`  ✓ ${name}`);
    ...
    if (origEnv === undefined) delete process.env.MISSIONCTL_DEFAULT_MODE;
    else process.env.MISSIONCTL_DEFAULT_MODE = origEnv;
...
test('reads .missionctl/config.json in cwd', (tmp) => {
  fs.mkdirSync(path.join(tmp, '.missionctl'));
  fs.writeFileSync(path.join(tmp, '.missionctl', 'config.json'),
    JSON.stringify({ defaultMode: 'lite' }));
  process.chdir(tmp);
  assert.strictEqual(getDefaultMode(), 'lite');
});

test('reads .missionctl.json in cwd', (tmp) => {
  fs.writeFileSync(path.join(tmp, '.missionctl.json'),
    JSON.stringify({ defaultMode: 'ultra' }));
  process.chdir(tmp);
  assert.strictEqual(getDefaultMode(), 'ultra');
});

test('.missionctl/config.json wins over .missionctl.json at same level', (tmp) => {
  fs.mkdirSync(path.join(tmp, '.missionctl'));
  fs.writeFileSync(path.join(tmp, '.missionctl', 'config.json'),
    JSON.stringify({ defaultMode: 'lite' }));
  fs.writeFileSync(path.join(tmp, '.missionctl.json'),
...
test('env var beats repo-local config', (tmp) => {
  ...
  fs.writeFileSync(path.join(tmp, '.missionctl', 'config.json'),
    JSON.stringify({ defaultMode: 'lite' }));
  process.chdir(tmp);
  process.env.MISSIONCTL_DEFAULT_MODE = 'ultra';
  assert.strictEqual(getDefaultMode(), 'ultra');
});

test('repo-local config beats user config', (tmp) => {
  // user config at XDG_CONFIG_HOME points to 'commit'
  fs.mkdirSync(path.join(tmpHome, 'missionctl'), { recursive: true });
  fs.writeFileSync(path.join(tmpHome, 'missionctl', 'config.json'),
  ...
  try {
    ...
    fs.rmSync(path.join(tmpHome, 'missionctl'), { recursive: true, force: true });
  }
});
...
test('malformed JSON in repo config falls through to default', (tmp) => {
  fs.mkdirSync(path.join(tmp, '.missionctl'));
  fs.writeFileSync(path.join(tmp, '.missionctl', 'config.json'), '{ not json');
  process.chdir(tmp);
  assert.strictEqual(getDefaultMode(), 'full');
});

test('refuses symlinked .missionctl.json (symmetric with readFlag policy)', (tmp) => {
  const real = path.join(tmp, 'real-config.json');
  fs.writeFileSync(real, JSON.stringify({ defaultMode: 'ultra' }));
  try {
    fs.symlinkSync(real, path.join(tmp, '.missionctl.json'));
  } catch (e) {
