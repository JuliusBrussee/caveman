...
- `compile.mjs` — zero-dep compiler: validates every profile (fail-closed) and emits
  `agents.json` + the CLI's embedded generated modules in either supported repository layout.
...
- **fail-closed**: unknown fields/enums/templates, command collisions, unsafe env
  ...
  file read/write.
- The compiler runs in the CLI build **and** test (`node scripts/compile-registries.mjs`); the generated
