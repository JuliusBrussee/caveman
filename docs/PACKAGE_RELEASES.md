...
Live check on 2026-08-10 returned 404 for `@caveman/agent`, `@caveman/sdk`,
`create-caveman-agent`, and PyPI `caveman-sdk`. A 404 shows no public artifact;
it does not prove current account ownership.
...
2. In public repository, create protected GitHub environments named `npm` and
   ...
   approval.
3. In npm, confirm founder owns `@caveman` scope and can create unscoped
   `create-caveman-agent`. Bootstrap each package under founder 2FA if npm cannot
   attach trusted publisher before first publication. Immediately configure its
   ...
   long-lived publish tokens. Never bootstrap from an unreviewed checkout.
4. In PyPI, create pending trusted publisher for project `caveman-sdk`: owner
...
|---|---|---|
| `sdk-ts-v1.0.0` | npm `@caveman/sdk` | `1.0.0` |
| `sdk-python-v1.0.0` | PyPI `caveman-sdk` | `1.0.0` |
| `agent-v0.1.0` | npm `@caveman/agent` | `0.1.0` |
| `create-agent-v0.1.0` | npm `create-caveman-agent` | `0.1.0` |
