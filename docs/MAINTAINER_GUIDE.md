# Maintainer guide — architecture reference

Full detail behind `CLAUDE.md`'s rules: directory layout, file ownership, CI sync, hook
system internals, skill system, evals, benchmarks. `CLAUDE.md` holds only the rules an
agent must follow when editing; this is the reference for how things actually work.

---

## Project overview

Caveman makes AI coding agents respond in compressed caveman-style prose — cuts 65%
output tokens (measured), full technical accuracy. Ships as Claude Code plugin, Codex
plugin, Gemini CLI extension, agent rule files for Cursor, Windsurf, Cline, Copilot, 40+
others via `npx skills`.
...
├── CLAUDE.md                    # Maintainer instructions (binding rules)
├── docs/MAINTAINER_GUIDE.md     # This file (architecture reference)
