...
//      - ~/.config/caveman/config.json (all platforms fallback)
//      - %APPDATA%\caveman\config.json (Windows fallback if ~/.config missing)
//   3. 'full'
...
function getConfigDirs() {
  ...
  if (process.env.XDG_CONFIG_HOME) {
    dirs.push(path.join(process.env.XDG_CONFIG_HOME, 'caveman'));
  }

  dirs.push(path.join(os.homedir(), '.config', 'caveman'));
...
function getConfigPath() {
  return path.join(getConfigDirs()[0], 'config.json');
}
...
module.exports = { getDefaultMode, getConfigDirs, getConfigPath, VALID_MODES, safeWriteFlag, readFlag };
