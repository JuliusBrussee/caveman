...
First, check for `.caveman/proposals/unlabeled-traffic-*.md`. If present, this
repo's Cave Plan already flagged unlabeled traffic and a Cave Agent draft PR
sent you here — you are on that PR's branch to apply it. Proceed as below, cite
the proposal file in your report, and leave the advisory file in place (the PR
review decides its fate, not you).
...
- **@caveman/sdk / caveman_cloud SDK**: per-trace `workflow` option, or
...
if so). Labeled spend appears on the dashboard at `/analytics/workflows` as
...
Lands at: <DASHBOARD>/analytics/workflows — each row appears as that workflow
