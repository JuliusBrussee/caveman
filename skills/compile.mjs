...
const REGISTRY_KEYS = new Set(["schema_version", "native_pack", "skills", "suites"]);
...
if (canonicalIDs.join("\n") !== skills.map((skill) => skill.id).join("\n")) {
  die(`registry ids do not match canonical skill dirs (registry: ${skills.map((skill) => skill.id).join(", ")}; dirs: ${canonicalIDs.join(", ")})`);
}
