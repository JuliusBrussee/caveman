...
$Flag = Join-Path $ClaudeDir ".missionctl-active"
...
if ([string]::IsNullOrEmpty($Mode) -or $Mode -eq "full") {
    [Console]::Write("${Esc}[38;5;172m[missionctl]${Esc}[0m")
} else {
    $Suffix = $Mode.ToUpperInvariant()
    [Console]::Write("${Esc}[38;5;172m[missionctl:$Suffix]${Esc}[0m")
}

# Savings suffix: on by default. Opt out via MISSIONCTL_STATUSLINE_SAVINGS=0.
# Reads a pre-rendered string written by missionctl-stats.js. Refuses reparse
# points and strips control bytes (matches statusline.sh hardening). Until
# /missionctl-stats has run at least once, the suffix file is absent and nothing
# is rendered — safe default for fresh installs.
if ($env:MISSIONCTL_STATUSLINE_SAVINGS -ne "0") {
    $SavingsFile = Join-Path $ClaudeDir ".missionctl-statusline-suffix"
