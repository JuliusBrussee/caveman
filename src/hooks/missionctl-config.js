#!/usr/bin/env node
// missionctl — shared configuration resolver
//
// Resolution order for default mode:
//   1. MISSIONCTL_DEFAULT_MODE environment variable
...
//      - $XDG_CONFIG_HOME/missionctl/config.json (any platform, if set)
//      - ~/.config/missionctl/config.json (macOS / Linux fallback)
//      - %APPDATA%\missionctl\config.json (Windows fallback)
//   4. 'full'
...
function getConfigDir() {
  if (process.env.XDG_CONFIG_HOME) {
    return path.join(process.env.XDG_CONFIG_HOME, 'missionctl');
  }
  ...
  return path.join(os.homedir(), '.config', 'missionctl');
}
...
// Walk up from `start` looking for a repo-local missionctl config. Returns the
// absolute path of the first match, or null. Stops at the filesystem root.
// Candidates per dir (first wins): .missionctl/config.json, .missionctl.json.
//
// Bounded to 64 levels to defend against symlink cycles on pathological mounts.
function findRepoConfigPath(start) {
  try {
    let dir = path.resolve(start || process.cwd());
    const candidates = ['.missionctl/config.json', '.missionctl.json'];
    for (let i = 0; i < 64; i++) {
...
function getDefaultMode() {
  ...
  const envMode = process.env.MISSIONCTL_DEFAULT_MODE;
...
// flag path (~/.claude/.missionctl-active) with a symlink to clobber other files.
//
...
//
// Set MISSIONCTL_DEBUG=1 to emit stderr diagnostics when flag writes are refused.
//
// Silent-fails on any filesystem error — the flag is best-effort.
function safeWriteFlag(flagPath, content) {
  const debug = process.env.MISSIONCTL_DEBUG === '1';
  try {
    ...
    try {
      ...
      if (lstat.isSymbolicLink()) {
        ...
        if (!realStat.isDirectory()) {
          if (debug) process.stderr.write(`[missionctl] safeWriteFlag: symlink target ${realFlagDir} is not a directory\n`);
          return;
        }
        if (typeof process.getuid === 'function') {
          if (realStat.uid !== process.getuid()) {
            if (debug) process.stderr.write(`[missionctl] safeWriteFlag: symlink target ${realFlagDir} owned by uid ${realStat.uid}, not current user ${process.getuid()}\n`);
            return;
          }
        } else {
          ...
          if (!normalizedReal.toLowerCase().startsWith(normalizedHome.toLowerCase() + path.sep) &&
            ...
            if (debug) process.stderr.write(`[missionctl] safeWriteFlag: symlink target ${normalizedReal} is outside home directory ${normalizedHome}\n`);
            return;
          }
        }
      } else {
    ...
    const tempPath = path.join(realFlagDir, `.missionctl-active.${process.pid}.${Date.now()}`);
...
// ($CLAUDE_CONFIG_DIR/.missionctl-history.jsonl).
//
  ...
  try {
    ...
    try {
      ...
      if (lstat.isSymbolicLink()) {
        ...
        if (typeof process.getuid === 'function') {
          if (realStat.uid !== process.getuid()) {
            if (debug) process.stderr.write(`[missionctl] appendFlag: symlink target ${realDir} owned by uid ${realStat.uid}\n`);
            return;
          }
        } else {
          const home = os.homedir();
