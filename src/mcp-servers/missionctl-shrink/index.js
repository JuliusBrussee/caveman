#!/usr/bin/env node
// missionctl-shrink — MCP middleware that proxies an upstream MCP server and
// compresses prose fields so the model sees fewer tokens.
//
// Usage:
//   missionctl-shrink <upstream-command> [...args]
//
// Example wrapping the filesystem MCP server:
//   "mcpServers": {
//     "fs-shrunk": {
//       "command": "npx",
//       "args": ["missionctl-shrink", "npx", "@modelcontextprotocol/server-filesystem", "/some/path"]
//     }
//   }
//
// Compression is applied to:
//   - "description" fields in tools/list, prompts/list, resources/list responses
//   - same boundaries as missionctl-compress: code, URLs, paths, identifiers preserved
//
...
//   MISSIONCTL_SHRINK_FIELDS   comma-separated extra field names to compress
//                           (default: description)
//   MISSIONCTL_SHRINK_DEBUG=1  log compression deltas to stderr
...
if (args.length === 0) {
  process.stderr.write('missionctl-shrink: missing upstream command.\n');
  process.stderr.write('Usage: missionctl-shrink <upstream-command> [...args]\n');
  process.exit(2);
}

const debug = process.env.MISSIONCTL_SHRINK_DEBUG === '1';
const fields = (process.env.MISSIONCTL_SHRINK_FIELDS || 'description')
...
upstream.on('error', err => {
  process.stderr.write(`missionctl-shrink: failed to spawn upstream: ${err.message}\n`);
  process.exit(1);
});
            ...
            if (out !== before) {
              ...
              if (debug) {
                process.stderr.write(
                  `[missionctl-shrink] ${arrayName}.${item.name || '?'}.${field}: ` +
