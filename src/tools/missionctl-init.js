#!/usr/bin/env node
// missionctl init — drop the always-on missionctl activation rule into a target
// repo for every IDE agent we support. Idempotent. Safe to re-run.
//
// Usage:
//   node src/tools/missionctl-init.js [target-dir] [--dry-run] [--force] [--only <agent>]
//   curl -fsSL https://raw.githubusercontent.com/dnl-re/missionctl/main/src/tools/missionctl-init.js | node - [args]
//
...
// existing memory files — that's the job of `/missionctl:compress`.

const fs = require('fs');
const path = require('path');

// Embedded so the tool works standalone (npx-style) without the src/rules/ dir.
// Mirrors src/rules/missionctl-activate.md verbatim — keep these in sync.
const RULE_BODY = `Respond with calm operational brevity. Keep all technical substance. Cut only filler.
...
Switch level: /missionctl lite|full|ultra|wenyan
Stop: "stop missionctl" or "normal mode"

Auto-Clarity: drop brevity for security warnings, irreversible actions, user confused. Resume after.

Boundaries: code/commits/PRs written normal.
`;

const SENTINEL = 'Respond with calm operational brevity';
...
// lives at bin/lib/openclaw.js; we require it lazily so missionctl-init.js
...
const AGENTS = [
  { id: 'cursor',   file: '.cursor/rules/missionctl.mdc',
    frontmatter: '---\ndescription: "missionctl mode — terse communication, ~75% fewer tokens, full technical accuracy"\nalwaysApply: true\n---\n\n',
    mode: 'replace' },
  { id: 'windsurf', file: '.windsurf/rules/missionctl.md',
    frontmatter: '---\ntrigger: always_on\n---\n\n',
    mode: 'replace' },
  { id: 'cline',    file: '.clinerules/missionctl.md',
    frontmatter: '',
    mode: 'replace' },
  ...
  { id: 'openclaw', description: '~/.openclaw/workspace/{skills/missionctl/, SOUL.md}',
    installer: 'openclaw' },
];
...
function loadRuleBody() {
  ...
  try {
    const local = path.join(__dirname, '..', 'rules', 'missionctl-activate.md');
...
function processOpenclaw(opts) {
  ...
  if (!helper) {
    return {
      ...
      label: 'x',
      detail: '~/.openclaw/workspace (helper unavailable in standalone curl|node mode — use `npx -y github:dnl-re/missionctl -- --only openclaw`)',
    };
  }
...
function help() {
  console.log(`missionctl init — drop always-on missionctl rule into a target repo

Usage: missionctl-init.js [target-dir] [--dry-run] [--force] [--only <agent>]
...
function main() {
  ...
  console.log(`📡 missionctl init — ${opts.target}${opts.dryRun ? ' (dry run)' : ''}\n`);
