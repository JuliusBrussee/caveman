#!/usr/bin/env python3
"""Benchmark missionctl vs normal Claude output token counts."""
...
SKILL_PATH = REPO_DIR / "skills" / "missionctl" / "SKILL.md"
...
def run_benchmarks(client, model, prompts, missionctl_system, trials):
    results = []
    total = len(prompts)
    ...
    for i, prompt_entry in enumerate(prompts, 1):
        ...
        for mode, system in [("normal", NORMAL_SYSTEM), ("missionctl", missionctl_system)]:
...
def compute_stats(results):
    ...
    for entry in results:
        ...
        missionctl_medians = statistics.median(
            [t["output_tokens"] for t in entry["missionctl"]]
        )
        savings = 1 - (missionctl_medians / normal_medians) if normal_medians > 0 else 0
        ...
        rows.append(
            {
                ...
                "missionctl_median": int(missionctl_medians),
    ...
    avg_missionctl = round(statistics.mean([r["missionctl_median"] for r in rows]))
...
def format_table(rows, summary):
    lines = [
        "| Task | Normal (tokens) | missionctl (tokens) | Saved |",
        "|------|---------------:|----------------:|------:|",
    ]
    for r in rows:
        label = format_prompt_label(r["id"])
        lines.append(
            f"| {label} | {r['normal_median']} | {r['missionctl_median']} | {r['savings_pct']}% |"
        )
    lines.append(
        f"| **Average** | **{summary['avg_normal']}** | **{summary['avg_missionctl']}** | **{summary['avg_savings']}%** |"
    )
    lines.append("")
    lines.append(
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark missionctl vs normal Claude")
    ...
    missionctl_system = load_missionctl_system()
    ...
    results = run_benchmarks(client, args.model, prompts, missionctl_system, args.trials)
