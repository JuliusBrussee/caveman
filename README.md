...
Multi-account: with no `GROK_HOME`, installer also walks `~/.grok-accounts/*`. Pin one home with `GROK_HOME=/path`. Uninstall: `node bin/install.js --uninstall` (or the same scoped line with `--uninstall`).
