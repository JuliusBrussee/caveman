...
high-density style — same answers, far fewer tokens. It only ever adds text to your *outgoing*
...
- `test/` — `directive.test.mjs` (pure-logic guards for the injection text).
...
## Conventions
- Build/test: `make product-test PRODUCT=extension` (runs `node --test`). There is no build step — MV3 loads the source directly.
- Package: `make extension-zip` → runs the tests, then zips only the shippable files to `dist/caveman-browser-<version>.zip`. The staged set is an explicit allowlist (`scripts/build-extension-zip.sh`) cross-checked against the manifest/CSS/HTML by `scripts/verify-extension-stage.mjs`.
