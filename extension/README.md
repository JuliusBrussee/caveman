...
replies in a compact, high-density style — *same answers, far fewer tokens.*
...
2. **Load unpacked** → pick this `public/extension/` folder.
...
```bash
make product-test PRODUCT=extension   # runs the directive unit tests
make extension-zip                    # builds the Chrome Web Store upload zip
```
