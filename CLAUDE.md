...
| Copilot CLI | Native plugin: `copilot plugin marketplace add JuliusBrussee/caveman && copilot plugin install caveman@caveman` (detected via `command:copilot`). `--with-init` writes repo instructions and registers the plugin in `.github/copilot/settings.json`. | Plugin alone: `/caveman` per session. With `--with-init`: repo instructions activate the voice. Caveman ships no compatible Copilot hook manifest. |
