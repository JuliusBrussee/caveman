...
| Grok CLI | Native install into `$GROK_HOME` (default `~/.grok`, override via `GROK_HOME` for tests/custom path): `rules/caveman.md` + marker-fenced block in `AGENTS.md` (always-on — Grok ignores SessionStart stdout) + skills under `skills/` + cavecrew agents under `agents/`. Requires local clone (copies from repo). | Yes — home rules + AGENTS.md load every session; default intensity **full** |
