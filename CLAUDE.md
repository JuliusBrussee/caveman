...
| Cursor | **Plugin:** `.cursor-plugin/plugin.json` ships `skills/` and a `sessionStart` primer via `hooks/hooks-cursor.json`. **Skills:** `npx skills add JuliusBrussee/caveman -a cursor -g` writes `~/.cursor/skills`. **Unified installer** (`node bin/install.js --only cursor`): copies Cavecrew agents into `~/.cursor/agents/`, installs `~/.cursor/rules/caveman.mdc` (`alwaysApply: true`), wires a user `sessionStart` hook, and registers `caveman` / `cavemem` in `~/.cursor/mcp.json` when binaries resolve. Missing `caveman-mcp`: `caveman setup --install`, then re-run `--only cursor`. **Other retrieve door:** `caveman mcp install cursor` when the MCP binary is present. Per-repo `.cursor/rules/caveman.mdc` via `--with-init`. **Not** a wrap profile (no gateway injection). | Skills per session. Plugin or user rule + hook in the IDE. Agents after a new chat. Per-repo rule via `--with-init`. Installer registers retrieve when `caveman-mcp` resolves; `caveman mcp install cursor` is the other door. |
