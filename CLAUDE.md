...
| Copilot CLI | Native plugin: `copilot plugin install JuliusBrussee/caveman` (detected by its plugin command, so AWS Copilot is excluded). `--with-init` writes repo instructions and registers the plugin in `.github/copilot/settings.json`. | Plugin alone: `/caveman` per session. With `--with-init`: repo instructions activate the voice. Caveman ships no compatible Copilot hook manifest. |
...
Copilot plugin hooks live in [`hooks.json` or `hooks/hooks.json`](https://github.com/github/docs/blob/872c2cee45930da1974cd022bf1faf68d3093750/content/copilot/reference/copilot-cli-reference/cli-plugin-reference.md#L278-L283), and successful command-hook stdout is [parsed as JSON](https://github.com/github/docs/blob/872c2cee45930da1974cd022bf1faf68d3093750/content/copilot/reference/hooks-reference.md#L793-L796). `caveman-activate.js` currently emits plain text, so adding only a manifest would not activate the voice.
