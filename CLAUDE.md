...
| Cursor | `npx skills add JuliusBrussee/caveman -a cursor -g` (default via `--only cursor`) writes skills into `~/.cursor/skills`. The unified installer also copies Cavecrew agents into `~/.cursor/agents/`, installs `~/.cursor/rules/caveman.mdc`, a user `sessionStart` hook, and MCP entries in `~/.cursor/mcp.json` when binaries resolve. Per-repo `.cursor/rules/caveman.mdc` via `--with-init`. **Not** a wrap profile (no gateway injection). | Skills per session. User rule + hook in IDE. Agents after a new chat. `caveman mcp install cursor` for retrieve MCP |
