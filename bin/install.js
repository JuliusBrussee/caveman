...
//
// Official config root is ~/.grok (override with GROK_HOME for tests / custom
// install path). We do not walk unofficial multi-home layouts.
...
// Resolve the single official Grok home: $GROK_HOME if set (tests / override),
// otherwise ~/.grok. Returns [] when the path is missing and create is false.
function discoverGrokHomes(opts = {}) {
  const home = grokDefaultHome();
...
function installGrok(ctx) {
  ...
  try {
    // Single official home (~/.grok or $GROK_HOME). No multi-home fan-out.
    for (const home of homes) {
      // Always-on rule
      ...
      appendGrokAgentsMd(path.join(home, 'AGENTS.md'), ruleBody, opts, note);
    }
...
function uninstall(ctx) {
  ...
  // from the official home (~/.grok or $GROK_HOME). Never mkdir on uninstall.
  {
    const homes = discoverGrokHomes({ dryRun: true });
