...
if (process.platform === "win32") {
  process.stderr.write("native hook benchmark currently requires a POSIX Unix socket\n");
  process.exit(2);
}

const root = join(dirname(fileURLToPath(import.meta.url)), "..");
const hook = join(root, "dist", "native-hook-fast.js");
const home = mkdtempSync(join(process.env.CAVEMAN_BENCH_TMPDIR ?? "/tmp", "caveman-native-bench-"));
