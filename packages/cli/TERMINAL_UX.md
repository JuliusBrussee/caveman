...
Existing opt-in/default-off `cli/v1 command_run` events cover exposure
(`help`, `tools`, `cloud`, `learn`), outcome (`learn implement` exit class and
duration), and failure class. `implement` is an allowlisted subcommand.
Telemetry never includes prompts, argv values, report contents, sink ids, or
paths. Owner reviews command adoption and errors weekly alongside local
ClickHouse/runtime evidence; no product decision uses opt-in telemetry alone.
