...
test("weekly refresh never blocks run start when entitlement service stalls", async () => {
  const f = await fixture({ responseDelayMs: 3000 });
  const started = Date.now();
  const out = await runCli(cli, ["wrap", "agent"], { env: f.env, cwd: f.dir, timeoutMs: 2500 });
  assert.equal(out.code, 0, out.stderr);
  assert.ok(Date.now() - started < 1500, "agent completion must not wait for entitlement response");
