...
const HELP = `caveman <agent>          run an agent on the layer  (aider | claude | codex | gemini | hermes | openclaw | opencode)
...
test("tools and cloud discovery keep named product order and caps", async () => {
  const isolated = isolatedCliEnv();
  try {
    ...
    assert.equal(tools.stdout.split("\n").filter((line) => /^\s{2}[a-z]/.test(line)).length, 15);
    assert.equal(cloud.stdout.split("\n").filter((line) => /^\s{2}[a-z]/.test(line)).length, 14);
  } finally {
    isolated.cleanup();
  }
});
