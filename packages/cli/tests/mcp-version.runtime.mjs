...
test("wrap probes current MCP once per resolved path and mtime, then auto-installs it", async () => {
...
`);
  ...
  assert.match(out.stderr, /installed streaming-compression recovery into OpenAI Codex CLI/);
  assert.ok(existsSync(join(f.caveHome, "mcp", "codex.json")), "auto-install marker missing");
});
