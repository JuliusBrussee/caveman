...
test("wrap config auto_recall installs the recall hook; plain wrap does not", async () => {
  ...
  const ups1 = JSON.parse(readFileSync(join(home1, ".claude", "settings.json"), "utf8")).hooks.UserPromptSubmit;
  assert.ok(ups1.some((e) => e.hooks.some((h) => (h.command || "").includes("mem recall-hook"))), "auto-recall must install the recall hook");
  assert.ok(existsSync(join(cave1, "recall-hooks", "claude.json")), "recall marker must be written");
  ...
  const settings2 = JSON.parse(readFileSync(join(home2, ".claude", "settings.json"), "utf8"));
  const ups2 = settings2.hooks.UserPromptSubmit || [];
  assert.ok(!ups2.some((e) => e.hooks.some((h) => (h.command || "").includes("mem recall-hook"))), "plain wrap must not add a recall hook");
});
