...
const { buildWrapSessionEvent } = await import(pathToFileURL(cli).href);
...
test("a persisted ON decision from an older prompt version gets the v3 disclosure once", async (t) => {
  const stub = startTelemetryStub();
  const port = await listenOrSkip(t, stub);
  if (port === null) return;
  const { env, home } = isolatedEnv({ CAVEMAN_TELEMETRY_URL: `http://127.0.0.1:${port}/telemetry` });
  ...
  writeFileSync(join(configDir, "config.json"), JSON.stringify({ telemetry: v2 }));
  ...
  assert.match(first.output, /anonymous usage stats on/, "a widened anonymous scope must re-disclose");
  assert.match(first.output, /session token totals/, "the disclosure must name the new token-total scope");
  const cfg = JSON.parse(readFileSync(join(configDir, "config.json"), "utf8"));
  assert.equal(cfg.telemetry.promptVersion, 3);
  assert.equal(cfg.telemetry.enabled, true);
  assert.equal(cfg.telemetry.anonymousId, v2.anonymousId, "re-disclosure must not rotate the anonymous id");
  assert.equal(cfg.telemetry.decidedAt, v2.decidedAt, "re-disclosure is not a new decision");
...
// The re-disclosure branch is the first code in ensureTelemetryDefault that ever
// rewrites a persisted decision — pin that an interactive run over a stale-version
// OPT-OUT leaves the config byte-identical, prints nothing, and sends nothing.
...
// ── wrap_session event builder (anonymous prompt-version-3 scope, pure) ────────

test("a measured wrap session builds a counts-only wrap_session event", () => {
  const event = buildWrapSessionEvent(
    "compress",
    { spans: 12, tokens_in: 480_000, compression_tokens_saved: 120_000, savings_usd: 4.6 },
    "claude",
  );
  // deepEqual pins the exact payload: counts only — any new field must be added
  // here deliberately, alongside the disclosure line that names the scope.
...
test("an unmeasured or empty session builds no wrap_session event", () => {
  assert.equal(buildWrapSessionEvent("compress", null), null, "missing summary → nothing measured, nothing claimed");
  assert.equal(buildWrapSessionEvent("observe", { spans: 0, tokens_in: 0 }), null, "empty session → no synthetic zeros");
  assert.equal(buildWrapSessionEvent("observe", { spans: 3 }), null, "no token count → nothing to report");
});
