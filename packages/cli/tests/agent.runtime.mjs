...
test("cave agent list hits the real endpoint and prints the server's proposals", async () => {
  ...
  const server = createServer((req, res) => {
    ...
    res.end(JSON.stringify({ data: [{ id: "prop-1", optimizer_id: "model-right-sizing", basis: "inferred", status: "drafting" }], next_cursor: null }));
  });
  ...
  assert.match(out.stdout, /"optimizer_id":\s*"model-right-sizing"/);
