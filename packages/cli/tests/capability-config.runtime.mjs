...
`, { mode: 0o755 });
  ...
  isolated.env.CAVE_GATEWAY_URL = `http://127.0.0.1:${22000 + Math.floor(Math.random() * 6000)}`;
...
test("config set invalid mode exits 2 and leaves file byte-identical", async () => {
  ...
  try {
    ...
    assert.equal(out.stderr, 'not a mode: "observe" — valid: compress | record | pixel   (status calls record "observe" while you are signed out)\n');
...
test("project allowlist contributes local capability sources only", async () => {
  ...
  writeGlobal(isolated.home, { think: { mode: "record", toon: true } });
  const cwd = mkdtempSync(join(tmpdir(), "cave-overlay-"));
  mkdirSync(join(cwd, ".caveman"), { recursive: true });
  writeFileSync(join(cwd, ".caveman", "config.json"), JSON.stringify({
    think: { mode: "compress", toon: false, shrink: false, pixel: { density: "conservative" } },
