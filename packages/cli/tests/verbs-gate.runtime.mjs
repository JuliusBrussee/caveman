...
import { loadSurfaces, scanSkillBody } from "../../skills/verbs-gate.mjs";
...
test("caveman-optimize binds SDK recovery to the issuing gateway", () => {
  const body = readFileSync(join(skillsDir, "caveman-optimize", "SKILL.md"), "utf8");
  assert.match(body, /GET \$GATEWAY\/sdk\/v1\/ccr\/<handle>/);
  assert.match(body, /same-project `sdk:write` API key/);
  assert.match(body, /`CAVE_API_KEY` supplied to this skill is only\s+`plan:read` and is insufficient/);
  assert.match(body, /those surfaces\s+resolve only handles issued by their local Engine\/CCR store/);
  assert.doesNotMatch(body, /recovered out of band via the CLI/);
});
