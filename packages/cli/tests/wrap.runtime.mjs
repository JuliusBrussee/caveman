...
test("wrap never persists delegate MCP even when capability is on", async () => {
  ...
  const run = async (configJson, tag) => {
    ...
    assert.equal(out.code, 0, out.stderr);
    return { home, existsSync, readFileSync };
  };
  const on = await run({ execute: { delegate: true }, wrap: { proxy: false, shrink: false, mcp: false, browse: false } }, "on");
  assert.equal(on.existsSync(join(on.home, ".codex", "config.toml")), false);
  assert.equal(on.existsSync(join(on.home, ".caveman", "mcp", "codex.caveman-delegate.json")), false);
  const off = await run({ wrap: { proxy: false, shrink: false, mcp: false, browse: false } }, "off");
  assert.equal(off.existsSync(join(off.home, ".caveman", "mcp", "codex.caveman-delegate.json")), false, "delegate must stay off by default");
});
