...
function baseEnv(home) {
  ...
  for (const k of ["CAVEMAN_PROXY_BIN", "CAVEMAN_ENGINE_BIN", "CAVEMAN_MCP_BIN", "CAVEMEM_BIN", "CAVEMAN_BROWSE_BIN"]) delete env[k];
  return env;
}
...
test("setup reports every missing binary, the install command, and exits non-zero", async () => {
  ...
  for (const name of ["caveman-proxy", "caveman-engine", "caveman-mcp", "cavemem", "caveman-browse"]) {
...
// The optional browse binary must not fail the gate: required-only present → 0.
test("setup treats caveman-browse as optional", async () => {
