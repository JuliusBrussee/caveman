...
// bare spawn stack trace. CAVE_GATEWAY_URL points at a free port so the "already
...
test("start renders a help panel when the proxy binary is missing", async () => {
  const env = {
    ...
    CAVE_GATEWAY_URL: "http://127.0.0.1:59071",
  };
...
test("wrap a resolvable agent injects the gateway base URLs", async () => {
  ...
  // Isolate HOME/CAVEMAN_HOME: `wrap claude` auto-loads the command-output hook into
  // the agent's config (the loadout), which would otherwise write to the real home.
