...
test("every shipped agent profile sends a protocol-correct request through the real compression engine", { timeout: 90_000 }, async () => {
  ...
  try {
    ...
    for (const id of expectedProfiles) {
      ...
      assert.ok(!request.raw.includes(longPrompt), `${id} sent original long prompt uncompressed`);
