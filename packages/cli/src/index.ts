...
function agentBuildCheck(argv: string[]): void {
  ...
  if (!bin) {
    console.error("caveman tools check: caveman-agent is not installed; install @caveman/agent in this project");
    process.exitCode = 1;
    return;
  }
  const result = spawnSync(bin, ["check", ...argv], {
    cwd: process.cwd(),
    env: process.env,
    stdio: "inherit",
  });
...
// Version 2 = the default-on era: telemetry is on unless the user turns it off
// (opt-out). A persisted version-1 decision — either answer to the old [y/N]
// prompt — is still honored forever; the default only fills the undecided gap.
// Version 3 widens the anonymous scope: end-of-wrap session token totals
// (wrap_session events). A persisted ON decision from an older version gets the
// updated disclosure re-printed once; an OFF decision is never touched.
...
const TELEMETRY_DISCLOSURE_LINE =
  "anonymous usage stats on — command counts and session token totals only, never prompts, code, or file paths · caveman telemetry off";
...
function telemetryState(): TelemetryRuntimeState {
  ...
  // anonymous id) + printed disclosure. (ADR 0032)
...
async function ensureTelemetryDefault() {
  ...
  // The anonymous scope grew (v3 adds wrap-session token totals): a persisted
  // ON decision from an older prompt version gets the updated disclosure
  // re-printed once, then the acknowledged version persists. The on/off
  // decision itself is never rewritten — an opt-out config stays byte-untouched,
  // and env kills / non-TTY never reach here (state.source is "env"/"runtime";
  // a CAVEMAN_TELEMETRY=1 opt-in deliberately gets no re-disclosure — an env
  // opt-in is an explicit act that has never printed one).
  if (state.source === "config" && state.state === "on" && state.config
      && state.config.promptVersion < TELEMETRY_PROMPT_VERSION) {
    // A failed persist must never block the command (read-only home, root-owned
    // config): the safe degradation is that the disclosure reprints next run.
    try {
      await saveTelemetryConfig({ ...state.config, promptVersion: TELEMETRY_PROMPT_VERSION });
    } catch {
      // fall through to the disclosure either way
    }
...
// Returns the send promise so a caller that exits explicitly (process.exit kills
// pending sockets) can await it; the 1.5s abort caps the delay either way. Most
// callers still fire-and-forget.
...
function runExternalSkillAdd(rest: string[]) {
  ...
  const result = spawnSync(npx, ["--yes", "skills", "add", ...args], {
    cwd: process.cwd(),
    env: process.env,
    stdio: "inherit",
  });
...
async function setupInstall(json: boolean, options: { continuing?: boolean } = {}) {
  ...
  // 0o700: proxy refuses CCR (sqlite parent) when ~/.caveman is group/world writable,
  // and a permissive umask (e.g. Homebrew's 002) would otherwise create it 0775.
...
type WrapOptions = { mode: WrapRuntimeMode; noProxy: boolean; toon: boolean; pixelModels?: string; pixelDensity?: string; noShrink: boolean; noMcp: boolean; noBrowse: boolean; delegate: boolean; minimal: boolean; autoRecall?: boolean; workflow?: string; command: string[] };
...
export function formatSessionSavings(
  ...
  // reported it — an older binary omits the field entirely (unknown, not zero), so
  // we fall through to the byte-safe wording rather than falsely accuse routing.
...
// buildWrapSessionEvent builds the content fields of the anonymous end-of-wrap
// token-total event (PURE, exported for unit tests). Prompt version 3 scope:
// aggregate counts the proxy already measured for the session line — request
// spans and provider-counted tokens_in — never prompts, argv, code, or file
// paths. Returns null when nothing was measured: a missing or empty summary
// reports nothing rather than a synthetic zero. (honesty rule: no-fake-savings)
export function buildWrapSessionEvent(
  kind: "observe" | "compress",
  s: ProxyObserveSummary | null,
  agentId?: string,
  ...
  const spans = n(s.spans);
  const tokensIn = n(s.tokens_in);
  if (spans <= 0 || tokensIn <= 0) return null;
  return {
    schema: "cli/v1",
    event: "wrap_session",
    command: "wrap",
...
// emitWrapSession sends the anonymous wrap_session token totals. It passes the
// same telemetrySendable choke point as every other emitter, and additionally
// honors the ZDR exclusion the connected wrap channel already promises ("wrap
// telemetry excluded by your data policy") — a ZDR org's token counts stay
// local on the anonymous channel too.
async function emitWrapSession(kind: "observe" | "compress", summary: ProxyObserveSummary | null, agentId?: string): Promise<void> {
  const event = buildWrapSessionEvent(kind, summary, agentId);
  if (!event) return;
  // CAVEMAN_OFFLINE=1 means "this wrap makes no external writes" — the same
  // switch that blocks syncAfterWrap two lines below the call site.
  if (wrapExternalWritesDisabled()) return;
  const state = telemetryState();
  if (!telemetrySendable(state)) return;
  if (readWrapEntitlement()?.telemetry_level === "zdr") return;
...
// printSessionSavings writes the block for what the local proxy measured this
// session (the caller reads the summary once and shares it with the anonymous
// wrap_session emit). A missing summary is itself unmeasurable, so it gets the
// same honest state-specific line rather than silence.
function printSessionSavings(kind: "observe" | "compress", s: ProxyObserveSummary | null, sinceISO: string, agentId?: string) {
  if (!s) {
...
// 60s covers a full pass over ~1GB / ~2k sessions of history (measured ~29s);
// 8s left the majority of a heavy month unmeasured and the reveal under-counted
// several-fold. The proxy stays time-boxed and honest if a machine still runs
// over: partial coverage prints "measured N of M" plus the under-count caveat.
const FIRST_RUN_SCAN_BUDGET_MS = 60_000;
...
// re-reads, would_cut counts each unique block once — those two carry no ratio.
// The stream line is the like-for-like figure against the sent total: each cut
// block weighted by the provider-counted turns it was observed riding in
// context, transcript arithmetic, never a projection. (no-fake-savings)
async function renderRetroReveal(retro: LearnRetro): Promise<void> {
  ...
  if (retro.would_cut_tokens > 0) {
    await countUpLine(
      (v) => `    caveman would have cut   ${green(`~${humanTokens(v)}`)} tokens of unique context`,
  ...
  if (retro.sessions_scanned < retro.sessions_total) {
    const budgetNote = retro.time_boxed ? " · time budget hit (newest sessions first), figures under-count" : "";
    process.stderr.write(dim(`    sessions without provider-counted usage are excluded from every total${budgetNote}\n`));
  }
  ...
  process.stderr.write(dim("    unique cut counts each block once · the rides-every-turn figure multiplies only by turns observed in your logs — what a re-sent token costs depends on provider caching\n\n"));
}
...
async function firstRunExperience(opts: { forced?: boolean } = {}): Promise<void> {
  ...
  // The retro budget bounds only the retro pass inside the proxy; cap the whole
  // child too so a huge cold history can never hold the first wrap for the
  // default 120s learn timeout. Must clear FIRST_RUN_SCAN_BUDGET_MS or the
  // child dies before the proxy's own time box. An explicit CAVE_LEARN_TIMEOUT wins.
  const priorLearnTimeout = process.env.CAVE_LEARN_TIMEOUT;
  if (priorLearnTimeout === undefined) process.env.CAVE_LEARN_TIMEOUT = "90";
  try {
    const planRaw = await proxyExecLearnAsync(
      ["learn", "scan", "--retro", "--retro-budget-ms", String(FIRST_RUN_SCAN_BUDGET_MS)],
...
async function runWrapped(bin: string, cmdArgs: string[], agent?: AgentProfile, opts: WrapOptions = { mode: "compress", noProxy: false, toon: true, noShrink: false, noMcp: false, noBrowse: false, delegate: false, minimal: false, command: [] }, codexSubscription = false) {
  ...
  if (result.summaryKind && result.sessionStart) {
    const summary = readProxyObserveSummary(result.sessionStart);
    printSessionSavings(result.summaryKind, summary, result.sessionStart, agent?.id);
    // Awaited: runWrapped exits explicitly below, which would kill an
    // un-awaited send before it leaves the socket.
    await emitWrapSession(result.summaryKind, summary, agent?.id);
  }
...
async function spawnWrapped(
  ...
  const ephemeralMcp = agent && (agent.id === "claude" || agent.id === "codex")
    && !opts.minimal && !opts.noMcp && wrapRecoveryEligible(opts)
    ? probeMcpBinary()
    : null;
  ...
  if (direct) {
    ...
    if (subscriptionCompressionActive) {
      ...
      if (subscriptionCompress && !desiredRecoveryViaMCP) process.stderr.write(dim(`→ ${SUBSCRIPTION_NO_RECOVERY_NOTE}\n`));
    }
  } else {
    ...
    if (subscriptionCompressionActive) {
      process.stderr.write(dim("→ subscription logins (Claude Pro/Max) compress locally too — live zone only; compressed turns are re-sent byte-identically so the provider cache stays warm\n"));
      process.stderr.write(dim(`→ ${SUBSCRIPTION_TOKENS_ONLY_NOTE}\n`));
    } else if (subscriptionCompress && !desiredRecoveryViaMCP) {
      process.stderr.write(dim(`→ ${SUBSCRIPTION_NO_RECOVERY_NOTE}\n`));
    }
  }
  ...
  try {
    env = direct
      ? { ...process.env }
      : agent?.id === "codex"
        ? buildCodexEphemeralWrapEnv(gw, codexSubscription, ephemeralMcpBinary, includeShrink)
        : buildWrapEnv(agent, gw);
    if (!direct && agent?.id === "claude") {
      const pluginDir = buildClaudeEphemeralPlugin(ephemeralMcpBinary, includeShrink, Boolean(opts.autoRecall));
  ...
  try {
    code = await new Promise<number>((resolve, reject) => {
      const child = spawn(bin, childArgs, { stdio: "inherit", env });
...
export function claudeCaveBuildEnv(): NodeJS.ProcessEnv {
  ...
  const checked = spawnSync(checker, ["check"], {
    cwd: process.cwd(),
    env: process.env,
    encoding: "utf8",
  });
...
function buildCodexEphemeralHome(gw: string, subscription: boolean, mcpBinary: string | undefined, includeShrink: boolean): string {
...
function buildCodexEphemeralWrapEnv(gw: string, subscription: boolean, mcpBinary: string | undefined, includeShrink: boolean): NodeJS.ProcessEnv {
  const env: NodeJS.ProcessEnv = { ...process.env };
  for (const key of WRAP_BASE_URL_ENV_VARS) delete env[key];
  env.CODEX_HOME = buildCodexEphemeralHome(gw, subscription, mcpBinary, includeShrink);
  return env;
}

function buildClaudeEphemeralPlugin(mcpBinary: string | undefined, includeShrink: boolean, includeRecall: boolean): string {
  ...
  if (mcpBinary) {
    writeFileSync(join(dir, ".mcp.json"), JSON.stringify({
      mcpServers: { caveman: { command: mcpBinary, args: [] } },
    }, null, 2) + "\n", { mode: 0o600 });
  }
  return dir;
}
...
function nativeHostProbe(agent: AgentProfile): { binary: string | null; launchable: boolean; version: string | null; error: string | null } {
  ...
  try {
    const out = spawnSync(binary, ["--version"], { encoding: "utf8", timeout: 3000 });
...
async function sync() {
  ...
  const practices = await syncLocalPracticeFindings(cfg);
  ...
  if (practices.kind === "synced") {
    console.log(`synced ${practices.findings} local practice findings · basis: inferred (tokens only; no payload evidence)`);
  }
}
...
async function syncAfterLogin() {
  try {
    ...
    const practices = await syncLocalPracticeFindings(cfg);
    if (out.kind === "synced") {
      console.error(`  ${mark("ok")} ${syncSavingsLine(out, true)}`);
    } else {
    ...
    if (practices.kind === "synced") {
      console.error(`  ${mark("ok")} synced ${practices.findings} local practice findings · inferred, tokens only`);
    }
  } catch (error) {
...
function resolveDelegateMcpCommand(): { command: string; args: string[] } | null {
  const candidates = [
    ...
    join(dirname(fileURLToPath(import.meta.url)), "..", "..", "agents", "delegate", "caveman-delegate-mcp.mjs"),
  ].filter(Boolean);
...
function installMcpClaude(mcp: { command: string; args: string[] }, serverName = "caveman"): boolean {
  ...
  try {
    execFileSync(claude, ["mcp", "add", "--scope", "user", serverName, "--", mcp.command, ...mcp.args], { stdio: "ignore" });
    return true;
  } catch (e) {
...
async function trial(rest: string[]) {
  ...
  try {
    ...
    const result = await spawnWrapped(resolvedAgent, extra, agent, { mode: "record", noProxy: true, toon: false, noShrink: false, noMcp: false, noBrowse: false, delegate: false, minimal: false, command }, trialURL);
    proxy.kill("SIGTERM");
...
type LearnRetro = {
  ...
  // Tool-output cuts re-weighted by the provider-counted turns each block was
  // observed riding in context (until compaction) — the like-for-like figure
  // against tokens_observed. Absent on older proxies and when nothing was cut.
...
async function status(argv: string[]) {
  ...
  } else if (runtime.recovery_via_mcp !== true) {
    states.push(fixedOffState("mcp-missing", OFF_STATES.mcpMissing));
  }
  if (!resolveGoBin("cavemem", "CAVEMEM_BIN")) states.push(fixedOffState("mem-missing", OFF_STATES.memMissing));
  if (entitlement?.telemetry_level === "zdr") states.push(fixedOffState("zdr", OFF_STATES.zdr));
