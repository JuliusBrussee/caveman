...
export const PRACTICE_REGISTRY = [
  ...
  {
    "id": "unlabeled-traffic",
    "family": "labeling",
    "title": "Label agent and workflow traffic",
    "predicate": {
      "harness": [
        "any"
      ],
      "payload_shape": "requests missing agent or workflow attribution",
      "wire_protocol": [
        "any"
      ]
    },
    "fix": {
      "prose": "Add stable agent and workflow labels at each workload entry point so later practices can target measured scopes.",
      "apply": "add_label"
    },
    "grader": {
      "type": "http_status",
      "fixture_template": "Held labeled request expected to succeed through the configured gateway path."
    },
    "experiment": {
      "method": "trial",
      "metric": "$/day band"
    ...
    "never_claim": "Never claim direct savings from labels; they only make attribution and later measurement possible.",
    "skill_render": {
      "eligible": true,
      "skill_name": "caveman-practice-unlabeled-traffic"
    }
  },
  {
    "id": "retry-loop-reduction",
    "family": "reliability",
    "title": "Break repeated request loops",
    "predicate": {
      "harness": [
        "any"
      ],
      "payload_shape": "same request repeated after failure or unusable response",
      "wire_protocol": [
        "any"
      ]
    },
    "fix": {
      "prose": "Retry only recoverable failures, honor server timing, cap attempts, and stop loops that replay the same expensive body.",
      "apply": "review_retries"
    },
    "grader": {
      "type": "http_status",
      "fixture_template": "Held recoverable and non-recoverable failures with expected terminal statuses and bounded attempts."
    },
    "experiment": {
      "method": "replay",
      "metric": "$/day band"
    ...
    "never_claim": "Never count the same failed request under both retry and error-spend practices.",
    "skill_render": {
      "eligible": true,
      "skill_name": "caveman-practice-retry-loop-reduction"
    }
  },
  {
    ...
    "predicate": {
      ...
      "payload_shape": "high-capability model serving narrow short-output work",
      "wire_protocol": [
        "any"
      ]
    },
    "fix": {
      "prose": "Evaluate a cheaper compatible model on held cases, then route only the passing workload behind rollback.",
      "apply": "right_size_model"
    },
    "grader": {
      "type": "llm_judge",
      "fixture_template": "Held representative cases with a rubric and a judge from a different model family."
    },
    "experiment": {
      "method": "canary",
      "metric": "$/day band"
    ...
    "never_claim": "Never infer quality parity from short outputs or catalog price alone.",
    "skill_render": {
      "eligible": true,
      "skill_name": "caveman-practice-model-right-sizing"
    }
  },
  {
    "id": "model-distillation-candidate",
    "family": "routing",
    "title": "Test a dedicated small model",
    "predicate": {
      "harness": [
        "any"
      ],
      "payload_shape": "high-volume narrow task with stable short output",
      "wire_protocol": [
        "any"
      ]
    },
    "fix": {
      "prose": "Train or configure a dedicated candidate, then compare it against the current model on the full held suite before any canary.",
      "apply": "distill_model"
    },
    "grader": {
      "type": "llm_judge",
      "fixture_template": "Held task suite covering normal, boundary, and refusal behavior with cross-family judging."
    },
    "experiment": {
      "method": "canary",
      "metric": "$/day band"
    ...
    "never_claim": "Never treat request volume as proof that a distilled model preserves task quality.",
    "skill_render": {
      "eligible": true,
      "skill_name": "caveman-practice-model-distillation-candidate"
    }
  },
  {
    "id": "fusion-routing",
    "family": "routing",
    "title": "Gate cheaper model routing",
    "predicate": {
      "harness": [
        "any"
      ],
      "payload_shape": "feature-safe traffic with cheaper compatible candidates",
      "wire_protocol": [
        "any"
      ]
    },
    "fix": {
      "prose": "Configure an explicit baseline and candidate allowlist, then promote only candidates that pass held quality checks.",
      "apply": "enable_cave_auto"
    },
    "grader": {
      "type": "llm_judge",
      "fixture_template": "Held workload with baseline and candidate outputs judged by a separate model family."
    },
    "experiment": {
      "method": "shadow",
      "metric": "$/day band"
    ...
    "never_claim": "Never call routing savings verified without a supported causal holdout or provider-measured counterfactual.",
    "skill_render": {
      "eligible": false,
      "skill_name": "caveman-practice-fusion-routing"
    }
  },
  {
  ...
  {
    "id": "context-exploration-offload",
    "family": "input_bloat",
    "title": "Isolate repository exploration",
    "predicate": {
      "harness": [
        "claude-code",
        "codex",
      ...
      "payload_shape": "solver context dominated by read-only search and file output",
      "wire_protocol": [
        "any"
      ]
    },
    "fix": {
      "prose": "Delegate broad read-only exploration to an isolated worker that returns compact path and line evidence.",
    ...
    "never_claim": "Never assume delegation is cheaper; prove the full solver-plus-worker token delta per pairing.",
    "skill_render": {
      "eligible": true,
      "skill_name": "caveman-practice-context-exploration-offload"
    }
  },
  {
    "id": "cache-write-read-churn",
    "family": "cache",
    "title": "Stop unstable cache writes",
    "predicate": {
      "harness": [
        "any"
      ],
      "payload_shape": "cache writes repeatedly exceed useful reads",
      "wire_protocol": [
        "anthropic",
        "openai",
        "any"
    ...
    "fix": {
      "prose": "Move cache boundaries below volatile content or remove a hint that creates entries without reuse.",
      "apply": "stabilize_prefix"
    },
    "grader": {
      "type": "token_threshold",
      "fixture_template": "Repeated held requests with a ceiling on cache-write tokens and a floor represented in experiment checks."
    },
    "experiment": {
      "method": "trial",
      "metric": "cache_hit_ratio"
    ...
    "never_claim": "Never assume low reuse means instability when request cadence may exceed cache lifetime.",
    "skill_render": {
      "eligible": true,
      "skill_name": "caveman-practice-cache-write-read-churn"
    }
  },
  {
    "id": "wasted-reasoning-suppression",
    "family": "routing",
    "title": "Lower unnecessary reasoning effort",
    "predicate": {
      "harness": [
        "any"
      ],
      "payload_shape": "reasoning-heavy calls returning trivial visible output",
      "wire_protocol": [
        "openai",
        "gemini",
        "any"
    ...
    "fix": {
      "prose": "Test a lower reasoning setting on the narrow workload and keep the change only when task quality holds.",
      "apply": "review_reasoning"
    },
    "grader": {
      "type": "llm_judge",
      "fixture_template": "Held trivial and deceptively hard cases judged across model families."
    },
    "experiment": {
      "method": "canary",
      "metric": "tokens_out"
    ...
    "never_claim": "Never infer wasted reasoning from short visible output when the response may contain a valuable tool call.",
    "skill_render": {
      "eligible": true,
      "skill_name": "caveman-practice-wasted-reasoning-suppression"
    }
  },
  {
    "id": "error-spend-reduction",
    "family": "reliability",
    "title": "Stop spend on failing calls",
    "predicate": {
      "harness": [
        "any"
      ],
      "payload_shape": "billed requests returning errors or unusable responses",
      "wire_protocol": [
        "any"
      ]
    },
    "fix": {
      "prose": "Fix the specific authentication, schema, rate-limit, timeout, or provider failure before changing retry behavior.",
      "apply": "review_reliability"
    },
    "grader": {
      "type": "http_status",
      "fixture_template": "Held success and known failure paths with expected statuses and no hidden retry."
    },
    "experiment": {
      "method": "replay",
      "metric": "$/day band"
    ...
    "never_claim": "Never count fast unbilled failures or the same spend already assigned to retry-loop reduction.",
    "skill_render": {
      "eligible": true,
      "skill_name": "caveman-practice-error-spend-reduction"
    }
  },
  {
    "id": "pixel-density",
    "family": "pixel_density",
    "title": "Test denser page images",
    "predicate": {
      "harness": [
        "any"
      ],
      "payload_shape": "pixel-mode pages below approved maximum density",
      "wire_protocol": [
        "any"
      ]
    },
    "fix": {
      "prose": "Raise density only through an approved canary with byte-exact originals retained for recovery.",
      "apply": "raise_pixel_density"
    },
    "grader": {
      "type": "llm_judge",
      "fixture_template": "Held visual-reading tasks covering dense text, tables, code, and retrieval of exact identifiers."
    },
    "experiment": {
      "method": "density-canary",
      "metric": "tokens_in"
    },
    "evidence": {
      "status": "unmeasured"
    },
    "never_claim": "Never price pixel savings from a local text tokenizer or claim hosted behavior before it is wired.",
    "skill_render": {
      "eligible": false,
      "skill_name": "caveman-practice-pixel-density"
    }
  },
  {
    "id": "anthropic-cache-breakpoints",
    "family": "cache",
    "title": "Use Anthropic automatic cache placement",
    "predicate": {
      "harness": [
        "any"
      ],
      "payload_shape": "growing conversation with a reusable prefix",
      "wire_protocol": [
        "anthropic"
      ]
    },
    "fix": {
      "prose": "Set top-level cache control when automatic movement of the last cacheable boundary matches the conversation shape.",
      "apply": "enable_gateway_optimizer"
    },
    "grader": {
      "type": "token_threshold",
      "fixture_template": "Repeated held conversation with a ceiling on uncached input after the first request."
    },
    "experiment": {
      "method": "trial",
      "metric": "cache_hit_ratio"
    ...
    "never_claim": "Never use automatic placement when volatile content would enter the cached prefix.",
    "skill_render": {
      "eligible": true,
  ...
  {
    "id": "gemini-explicit-cache",
    "family": "cache",
    "title": "Register reusable Gemini context",
    "predicate": {
      "harness": [
        "any"
      ],
      "payload_shape": "large stable context reused across shorter prompts",
      "wire_protocol": [
        "gemini"
      ]
    },
    "fix": {
      "prose": "Create explicit cached content only for supported models and reuse it while the same context remains valid.",
      "apply": "enable_gateway_optimizer"
    },
    "grader": {
      "type": "exact_match",
      "fixture_template": "Held queries against cached and uncached copies of the same context with normalized expected answers."
    },
    "experiment": {
      "method": "trial",
      "metric": "cache_hit_ratio"
    ...
    "never_claim": "Never claim this optimizer works in Caveman until the registration path emits a real optimizer identifier.",
    "skill_render": {
      "eligible": true,
      "skill_name": "caveman-practice-gemini-explicit-cache"
    }
  },
  {
    "id": "gemini-cache-ttl-right-sizing",
    "family": "cache",
    "title": "Match Gemini cache TTL to reuse",
    "predicate": {
      "harness": [
        "any"
      ],
      "payload_shape": "explicit cached content with known reuse window",
      "wire_protocol": [
        "gemini"
      ]
    },
    "fix": {
      "prose": "Set cache lifetime from observed reuse and delete or expire content when its valid window ends.",
      "apply": "stabilize_prefix"
    },
    "grader": {
      "type": "cost_threshold",
      "fixture_template": "Held reuse schedule comparing supported cache lifetimes under one cost ceiling."
    },
    "experiment": {
      "method": "replay",
      "metric": "$/day band"
    ...
    "never_claim": "Never keep explicit cached content longer than its measured reuse or validity window.",
    "skill_render": {
      "eligible": true,
      "skill_name": "caveman-practice-gemini-cache-ttl-right-sizing"
    }
  },
  {
  ...
  {
    "id": "anthropic-cache-breakpoints-enable",
    "family": "cache",
    "title": "Enable Anthropic cache breakpoints",
    "predicate": {
      "harness": [
        "any"
      ],
      "payload_shape": "Anthropic traffic paying the cache write premium without amortizing reads",
      "wire_protocol": [
        "anthropic"
      ]
    },
    "fix": {
      "prose": "Enable the provider-native cache-breakpoint optimizer so the stable prefix bills at the cache-read rate; the transform touches the upstream request only.",
      "apply": "enable_gateway_optimizer"
    },
    "grader": {
      "type": "token_threshold",
      "fixture_template": "Held request pair asserting the adapter's model-visible projection is byte-identical with and without cache_control markers."
    },
    "experiment": {
      "method": "canary",
      "metric": "cache_hit_ratio"
    ...
    "never_claim": "Never book verified savings from the flip itself; only the existing provider-causal ledger path mints dollars, after activation on real traffic.",
    "skill_render": {
      "eligible": false,
      "skill_name": "caveman-recipe-anthropic-cache-breakpoints-enable"
    }
  },
  {
    "id": "bedrock-cache-points-enable",
    "family": "cache",
    "title": "Enable Bedrock cache points",
    "predicate": {
      "harness": [
        "any"
      ],
      "payload_shape": "Bedrock Anthropic Claude traffic paying the cache write premium without amortizing reads",
      "wire_protocol": [
        "bedrock"
      ]
    },
    "fix": {
      "prose": "Enable the provider-native cache-point optimizer so the stable prefix bills at the cache-read rate; the transform touches the upstream request only.",
      "apply": "enable_gateway_optimizer"
    },
    "grader": {
      "type": "token_threshold",
      "fixture_template": "Held request pair asserting the adapter's model-visible projection is byte-identical with and without cachePoint/cache_control markers."
    },
    "experiment": {
      "method": "canary",
      "metric": "cache_hit_ratio"
    ...
    "never_claim": "Never book verified savings from the flip itself; only the existing provider-causal ledger path mints dollars, after activation on real traffic — and only under the Bedrock method, never the Anthropic-direct one.",
    "skill_render": {
      "eligible": false,
      "skill_name": "caveman-recipe-bedrock-cache-points-enable"
    }
  },
  {
    "id": "cache-dynamic-content-relocation",
    "family": "cache",
    "title": "Relocate dynamic content out of the cacheable prefix",
    "predicate": {
      "harness": [
        "any"
      ],
      "payload_shape": "churning content interleaved with the stable system or tool prefix",
      "wire_protocol": [
        "anthropic",
        "openai"
      ]
    ...
    "fix": {
      "prose": "Move churning content out of the cacheable region into a trailing user block so the stable prefix stops being re-billed at the write premium.",
      "apply": "stabilize_prefix"
    },
    "grader": {
      "type": "token_threshold",
      "fixture_template": "Held multi-turn session asserting the relocated prompt keeps answers intact while the prefix hash stays stable across turns."
    },
    "experiment": {
      "method": "replay",
      "metric": "cache_hit_ratio"
    ...
    "never_claim": "Never present the modeled relocation benefit as measured; the PR changes model-visible bytes and stays inferred until eval-gated rollout.",
    "skill_render": {
      "eligible": false,
      "skill_name": "caveman-recipe-cache-dynamic-content-relocation"
    }
  },
  {
    "id": "retry-idempotency-guard",
    "family": "reliability",
    "title": "Guard identical tool calls with an idempotency check",
    "predicate": {
      "harness": [
        "any"
      ],
      "payload_shape": "session-linked agent traffic re-running identical tool calls within seconds",
      "wire_protocol": [
        "anthropic"
      ]
    },
    "fix": {
      "prose": "Add an idempotency guard to the tool layer: remember each call's input and answer for a short window and return the remembered answer instead of re-running an identical call.",
      "apply": "add_idempotency_guard"
    },
    "grader": {
      "type": "tool_not_called",
      "fixture_template": "Held session replay asserting the second identical tool call within the window is answered from the guard without re-executing the tool, while a changed input still executes."
    },
    "experiment": {
      "method": "replay",
      "metric": "per_turn_duplicate_hash_share"
    },
    "evidence": {
      "status": "unmeasured"
    },
    "never_claim": "Never present the post-merge fall in duplicate-hash share as verified savings; it is an observed before/after correlation, stated as such in the PR, and mints nothing.",
    "skill_render": {
      "eligible": false,
      "skill_name": "caveman-recipe-retry-idempotency-guard"
    }
  },
  {
    "id": "mcp-catalog-prune",
    "family": "input_bloat",
    "title": "Prune tools the agent never uses from the catalog",
    "predicate": {
      "harness": [
        "any"
      ],
      "payload_shape": "requests re-sending a tool catalog with provider-counted overhead and never-invoked tools",
      "wire_protocol": [
        "anthropic"
      ]
    },
    "fix": {
      "prose": "Remove never-used tools (or whole MCP servers) from the declared catalog, or defer their loading so they are described to the model only when needed.",
      "apply": "prune_tool_catalog"
    },
    "grader": {
      "type": "tool_not_called",
      "fixture_template": "Held session replay asserting the pruned catalog still answers the held tasks — the removed tools were never called in the fixture set, and every previously-passing task still passes."
    },
    "experiment": {
      "method": "replay",
      "metric": "tools_schema_tokens"
    },
    "evidence": {
      "status": "unmeasured"
    },
    "never_claim": "Never present the unused-catalog overhead as savings earned; the catalog only shrinks when the customer's own client changes, and the figure stays inferred until then.",
    "skill_render": {
      "eligible": false,
      "skill_name": "caveman-recipe-mcp-catalog-prune"
    }
  },
  {
    "id": "wake-gate",
    "family": "reliability",
    "title": "Skip scheduled runs whose input is unchanged",
    "predicate": {
      "harness": [
        "any"
      ],
      "payload_shape": "near-periodic runs re-sending a byte-identical input hash",
      "wire_protocol": [
        "any"
      ]
    },
    "fix": {
      "prose": "Add a wake gate to the scheduler: compute the input first, compare its hash to the last run's, and skip the call when nothing changed.",
      "apply": "add_wake_gate"
    },
    "grader": {
      "type": "tool_not_called",
      "fixture_template": "Held scheduler replay asserting an unchanged-input tick makes no model call while a changed input still runs."
    },
    "experiment": {
      "method": "replay",
      "metric": "duplicate_request_share"
    },
    "evidence": {
      "status": "unmeasured"
    },
    "never_claim": "Never present the repeat-run spend as recovered until the unchanged runs actually stop being sent; a gate proposal mints nothing.",
    "skill_render": {
      "eligible": false,
      "skill_name": "caveman-recipe-wake-gate"
    }
  },
  {
    "id": "loop-bound-guard",
    "family": "reliability",
    "title": "Bound the agent's tool-call loop",
    "predicate": {
      "harness": [
        "any"
      ],
      "payload_shape": "session tool-call graph with re-traversed cycles",
      "wire_protocol": [
        "anthropic"
      ]
    },
    "fix": {
      "prose": "Cap retries per step and total laps per task; stop or escalate when the agent revisits the same tool-call cycle.",
      "apply": "add_loop_bound"
    },
    "grader": {
      "type": "tool_sequence",
      "fixture_template": "Held session replay asserting the bounded loop stops after the cap while a productive sequence still completes."
    },
    "experiment": {
      "method": "replay",
      "metric": "per_turn_duplicate_hash_share"
    ...
    "never_claim": "Never present loop-repeat spend as recovered until a bound actually stops the extra laps; excluded failed-attempt retries are never counted as waste.",
    "skill_render": {
      "eligible": false,
      "skill_name": "caveman-recipe-loop-bound-guard"
    }
  },
  {
    "id": "tool-result-cap",
    "family": "input_bloat",
    "title": "Cap oversized tool answers at the source",
    "predicate": {
      "harness": [
        "any"
      ],
      "payload_shape": "tool answers far above their own tool's median size",
      "wire_protocol": [
        "anthropic"
      ]
    },
    "fix": {
      "prose": "Cap or summarize oversized tool answers before they enter context — return what the agent needs, not everything the tool found.",
      "apply": "cap_tool_results"
    },
    "grader": {
      "type": "token_threshold",
      "fixture_template": "Held replay asserting capped answers stay under the per-tool ceiling while every previously-passing task still passes."
    },
    "experiment": {
      "method": "replay",
    ...
    "never_claim": "Never present the above-median answer overhead as recovered; sizes are byte-based estimates and the cap must ship in the customer's own tool layer.",
    "skill_render": {
      "eligible": false,
      "skill_name": "caveman-recipe-tool-result-cap"
    }
  },
  {
    "id": "exploration-offload-directive",
    "family": "input_bloat",
    "title": "Install the exploration-offload directive",
    "predicate": {
      "harness": [
        "claude-code",
        "codex",
    ...
    "fix": {
      "prose": "Install the managed exploration-offload directive in the agent's instructions file: run `caveman tools hooks install --directive exploration-offload-directive` so broad read-only exploration is delegated to an isolated explorer that returns compact path-and-line evidence instead of full file dumps. Undo with `caveman tools hooks uninstall --directive exploration-offload-directive`.",
      "apply": "install_exploration_offload_directive"
    },
    "grader": {
      "type": "localization_f1",
      "fixture_template": "Held repository-localization tasks graded A/B (directive installed vs not) with expected files and line neighborhoods; the directive arm must hold localization quality."
    },
    "experiment": {
      "method": "local_ab",
      "metric": "tokens_in"
    ...
    "never_claim": "Never state a dollar figure or per-day dollar rate for this directive: the offload can induce extra tool calls, so the only honest claim is the input-token delta on the measured turns — inferred, tokens only, never verified.",
    "skill_render": {
      "eligible": false,
      "skill_name": "caveman-recipe-exploration-offload-directive"
    }
  },
  {
    "id": "deferred-tool-loading",
    "family": "input_bloat",
    "title": "Install the deferred-tool-loading directive",
    "predicate": {
      "harness": [
        "claude-code",
        "codex",
    ...
    "fix": {
      "prose": "Install the managed deferred-tool-loading directive in the agent's instructions file: run `caveman tools hooks install --directive deferred-tool-loading` so tools outside the task's core set are loaded through a search-then-load surface when needed instead of the full catalog riding every request. Undo with `caveman tools hooks uninstall --directive deferred-tool-loading`.",
      "apply": "install_deferred_tool_loading_directive"
    },
    "grader": {
      "type": "tool_not_called",
      "fixture_template": "Held session replay asserting the deferred catalog still answers the held tasks — every previously-passing task still passes, and deferred tools load on demand when a task needs them."
    },
    "experiment": {
      "method": "local_ab",
      "metric": "tokens_in"
    ...
    "never_claim": "Never present the unused-catalog overhead as savings earned, and never state a dollar figure for this directive: deferral can induce extra loading turns, so the only honest claim is the token delta on measured turns — inferred, tokens only, never verified. The F3 evidence names the overhead; it does not price this fix.",
    "skill_render": {
      "eligible": false,
      "skill_name": "caveman-recipe-deferred-tool-loading"
    }
  }
] as const;
