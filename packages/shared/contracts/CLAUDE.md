...
- `schemas/agent-run-receipt.schema.json` — `caveman.agent.run-receipt.v1`,
  ...
  anonymous CLI event and not a signed verified-savings receipt
- `schemas/continuous-improvement-report.schema.json` — versioned end-to-end
  improvement report; E4 replay is structurally pinned to `inferred` /
  `replayed_counterfactual`, never verified savings
