...
const USAGE = "usage: npm create caveman-agent@latest <project> [--provider anthropic|openai|google] [--no-install]";
...
function projectFiles(name: string, provider: Provider): Record<string, string> {
  return {
    ...
    "src/agent.ts": `import { agent, auto, schema, tool } from "@caveman/agent";
...
`,
    "src/run.ts": `import { run } from "@caveman/agent";
import definition from "./agent.js";
...
const result = await run(definition, "Reply with exactly: ready", {
  budget: {
    // Hard cap. The run reserves each call's worst case against this and can
    // never spend past it.
    maxUsd: 0.5,
    // What happens when the cap binds. "compact" (the default) compresses the
    // context and carries on, spending the compaction inside the same cap.
...
`,
    "evals/smoke.eval.ts": `import { eval as defineEval } from "@caveman/agent";
...
`,
    "caveman.config.ts": `import { defineBuild } from "@caveman/agent/build";
...
(\`maxUsd\` or \`maxTokens\`), reserves each call's worst case against it, and can
never spend past it. When the cap binds, \`budget.onExhausted\` decides what
happens: \`"compact"\` compresses the context and carries on inside the same cap,
\`"stop"\` clamps the output and stops. Either way the run returns partial work
plus a \`stopReason\` and a receipt — never a throw.
