...
export interface RunOptions {
   ...
   * Ignored on the gateway path and for a caller-supplied `streamFn`, where the
   * local store is not the billing authority.
   */
...
export async function runAgent(
  ...
  return runAgentWithOptions(
    ...
    input,
    options,
    rootExecutionContext(definition, options.maxCostUsd),
  );
}
...
async function* streamAgentInternal(
  ...
  try {
    ...
    const caveRoute = await resolveCaveRoute(gatewayURL, options, efficiencyPlan !== undefined);
    ...
    //
    // Which credential pays is what decides this, so two paths are outside the
    // question rather than exempted from it. A caller-supplied streamFn owns
    // its own transport and never asks Pi to authenticate anything, so the
    // local login says nothing about how the run is billed. A gateway-routed
    // run bills the account key the gateway holds, not the developer's local
    // OAuth session — refusing there would refuse the flagship paid path to
    // anyone signed in to Claude Pro on the same machine.
    if (budgetMeter?.denomination === "usd" &&
        options.streamFn === undefined && !gatewayActive) {
    ...
    const accountProviderMessage = (message: AssistantMessage): void => {
      ...
      try {
        ...
        if (budgetReservation !== undefined) {
          budgetMeter?.settle(
            budgetReservation,
            budgetMeter.denomination === "usd" ? usage.catalogCostUsd : usage.totalTokens,
          );
        }
        ...
        if (budgetReservation !== undefined) {
          budgetMeter?.settle(budgetReservation, budgetReservation.amount);
        }
    ...
    const streamFn: StreamFn = async (selected, context, streamOptions) => {
      ...
      try {
        ...
        if (orphaned?.reservation !== undefined) budgetMeter?.cancel(orphaned.reservation);
        throw spendFailure;
      }
      ...
      // and nothing was consumed. Each attempt takes its worst case out of a
      // retry allowance denominated in the run's own budget, so an error storm
      // exhausts the allowance instead of the wallet, and the backoff is
      // deterministic — a breaker has to be reproducible.
      if (breakers?.config.retryMaxSpend === undefined || decided.reservation === undefined) {
        return streamOnce();
      }
      return retryModelCall(streamOnce, breakers, decided.reservation.amount);
    };
    ...
    const pi = new Agent({
      ...
      prepareNextTurnWithContext: async (turn) => {
        ...
        // Trigger: the moment the reserve check fails, not a threshold.
        const fits = callFitsBudget(budgetMeter, callCeilingFor(
          routedModel,
          instructions,
...
function runtimeHeaders(
  ...
  if (process.env.CAVE_API_KEY) headers["x-cave-api-key"] = process.env.CAVE_API_KEY;
  if (buildIdentity) {
...
async function compactContext(input: {
  ...
  headers: ProviderHeaders | undefined;
  /**
   * The ancestor spend ledgers this run reserves against. A compaction is a
   * provider call like any other, so it holds against every subagent wallet
   * above it too — otherwise a child could compact its way past the
   * `maxCostUsd` its parent granted it and `cave_subagent_cost_budget` would
   * never fire.
   */
  ...
  if (fitsAfterEviction) {
    if (!evictionHelped) return undefined;
  ...
  //
  // The consequence is deliberate and worth stating: with the summarizer on the
  // run's own model its input ceiling is essentially the working call's, so at
  // the trigger — the moment that working call stops fitting — the precondition
  // below cannot clear. The rung is reachable when the summarizer is genuinely
  // cheaper than the working model, and otherwise the run clamps and stops.
  // That is the honest arithmetic rather than a discount we cannot evidence.
  ...
  // The ancestor wallets get their hold too. A ledger that cannot cover this
  // call declines the compaction here rather than throwing: refusing to compact
  // is a between-calls decision, and the run continues to the clamp rung.
  ...
  try {
    ...
    unpriced = !usage.priced;
    // Settling against the ancestor wallets is what makes a child's compaction
    // count towards the maxCostUsd its parent granted it.
    ...
    // tell the run and its ancestor wallets the evidence is incomplete.
  ...
  // A compaction that pushed an ancestor wallet past its cap is that wallet's
  // failure to raise, exactly as a working call would be.
...
async function retryModelCall(
  ...
  for (;;) {
    try {
      ...
      attempt++;
      if (!breakers.allowRetry(worstCaseSpend, attempt)) throw error;
...
export async function caveGatewayReady(
  ...
  return await runtimeReady(gatewayURL, fetchImpl);
}
...
export async function resolveCaveRoute(
  ...
  if (options.caveRoute !== undefined) return options.caveRoute;
  if (options.cave === "off") {
    if (planPresent) {
      throw new Error(
    ...
    return { useGateway: false };
  }
  if (options.ensureRuntime === false) {
    try {
      const url = new URL(gatewayURL);
      if (isLoopbackHostname(url.hostname)) return { useGateway: true };
      ...
      await ensureCaveRuntime(gatewayURL, options.fetch ?? globalThis.fetch);
      return { useGateway: true };
    } catch (error) {
  ...
  if (useCache) {
    ...
    if (cached !== undefined && now - cached.at < GATEWAY_PROBE_TTL_MS) {
      if (cached.ready) return { useGateway: true };
      if (planPresent) {
...
type GatewayProbe = { ready: boolean; reason: string };
...
async function probeGateway(
  ...
  try {
    await ensureCaveRuntime(gatewayURL, fetchImpl);
...
export async function ensureCaveRuntime(gatewayURL: string, fetchImpl: typeof globalThis.fetch): Promise<void> {
  const url = new URL(gatewayURL);
  ...
  if (!loopback) {
    ...
    if (await gatewayIdentified(gatewayURL, fetchImpl)) return;
    throw new Error(
      `cave_gateway_identity_unverified: ${gatewayURL}/health/ready did not identify as caveman-proxy`,
    );
  }
  if (await runtimeReady(gatewayURL, fetchImpl)) return;
...
async function runtimeReady(gatewayURL: string, fetchImpl: typeof globalThis.fetch): Promise<boolean> {
  if (!(await gatewayIdentified(gatewayURL, fetchImpl))) return false;
  return await localProxyOwned(new URL(gatewayURL));
}
...
async function gatewayIdentified(gatewayURL: string, fetchImpl: typeof globalThis.fetch): Promise<boolean> {
  try {
    ...
    return value !== null && typeof value === "object" && !Array.isArray(value) &&
      ...
      (value as Record<string, unknown>).service === "caveman-proxy" &&
      (value as Record<string, unknown>).schema === "caveman.proxy.health.v1" &&
      Number.isSafeInteger((value as Record<string, unknown>).adapters) &&
      Number((value as Record<string, unknown>).adapters) > 0;
  } catch {
    return false;
  }
}
...
function proxyBinaryCandidates(): string[] {
  ...
  const home = process.env.CAVEMAN_HOME ?? resolve(homedir(), ".caveman");
  return [...new Set([resolve(home, "bin", "caveman-proxy"), "caveman-proxy"])];
}
...
async function validatedProxyState(binary: string, port: number): Promise<boolean> {
  return await new Promise<boolean>((done) => {
    ...
    const child = spawn(binary, ["status", "--json", "--port", String(port)], {
      env: buildRuntimeControlEnv(),
