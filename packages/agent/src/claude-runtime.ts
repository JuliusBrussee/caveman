...
async function runClaudeAgentWithOptions(
  ...
  // Runtime-gated denomination. This lane runs on whatever
  // credential the Claude Agent SDK finds: an API key bills per token at list
  // price, a Claude subscription does not bill this run at all. A dollar cap on
  // a subscription would be fiction, so it fails closed instead — cap the run
  // in tokens through the agent's own output contract.
  if (options.maxBudgetUsd !== undefined && !process.env.ANTHROPIC_API_KEY) {
  ...
  try {
    ...
    if (result !== undefined) {
      try {
        usage = usageFromSDKResult(result, selected);
      } catch (error) {
...
// failed run too.
function usageFromSDKResult(result: SDKResultMessage, model: string) {
