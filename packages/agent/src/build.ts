...
function lockableValue(value: unknown, seen: WeakSet<object>): unknown {
  ...
  try {
    ...
    const implementationSource = Reflect.get(
      value,
      Symbol.for("@caveman/agent:tool-implementation-source"),
    );
...
// The full supported grader taxonomy. MUST match public/evals
// `SUPPORTED_GRADER_TYPES` (the single source of truth, now exported from
// `@caveman/evals`) — a valid grader from public/evals was previously rejected
// here as unknown because only 4 of the 16 types were listed.
