/**
 * `@caveman/agent/code` — the Caveman coding agent.
 *
...
function codingTools(
  ...
  const bashTool = tool({
    ...
    async execute(input, signal) {
      ...
      if (run.spawnFailed) throw new Error("caveman-code: /bin/sh is not available");
      const body = [
    ...
    const killTree = () => {
      if (child.pid !== undefined) {
        try {
          ...
          // The group is already gone, or this platform has none.
        }
      }
      child.kill("SIGKILL");
    };
