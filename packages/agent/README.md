...
published to npm. Registry commands below describe release surface and currently
require local tarballs until `@caveman/agent` and `create-caveman-agent` publish.
...
and network still required. Until packages publish, use local tarballs from this
repository. On machine that has never seen Caveman, first run looks like this:
...
`@caveman/agent/adapters` when foundation checks pass.

Adapter frameworks are optional exact-pinned peers; install only lane used:

```bash
npm install @caveman/agent ai@7.0.43
npm install @caveman/agent @mastra/core@1.55.0
npm install @caveman/agent eve@0.29.2
```
...
```ts
import { createConversation, run } from "@caveman/agent";
...
## The coding agent (`@caveman/agent/code`)
...
```ts
import { createCodingAgent, runCodingSession } from "@caveman/agent/code";
...
Advanced build config lives in `@caveman/agent/build`. Claude public execution
lives in `@caveman/agent/claude`. `@caveman/agent/adapters` exports executable
...
- `runClaudeAgent` from `@caveman/agent/claude` (unlocked/inferred only)
- `createCodingAgent`, `startCodingSession`, `runCodingTurn`, `runCodingSession`,
  `defaultCodingPlan`, `proveRecovery` from `@caveman/agent/code`
- `createVercelAISDKAdapter`, `createEveAdapter`, `createMastraAdapter` from
  `@caveman/agent/adapters`
