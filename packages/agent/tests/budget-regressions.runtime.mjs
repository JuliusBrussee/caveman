...
test("B2: a subagent's compaction propagates into the parent's totals", async () => {
  const child = agent({
    id: "b2-child",
    instructions: "Answer.",
    model: "anthropic/claude-haiku-4-5",
...
// ---------------------------------------------------------------------------
// M5 — the compaction call bypassed reserveProviderSpend/settleProviderSpend,
// so it never touched the ancestor subagent wallets that cave_subagent_cost_
...
test("M5: a subagent's compaction is accounted against its wallet", async () => {
  ...
  const result = await run(parent, "go", {
    ...
    streamFn: (selected, context) => {
      ...
      if (context.messages.some((message) =>
        typeof message.content === "string" && message.content === "sub")) {
        childCalls++;
        if (childCalls > 20) {
  ...
  // Unconditional, and it says what is actually true: a subagent does not
  // compact under the current cap structure. subagent() derives the wallet and
  // the legacy ledger from the same maxCostUsd, and the ledger prices every
  // call at the model's whole context window — which is exactly the budget's
  // saturated byte ceiling. So the turn the wallet triggers a compaction on is
  // the same turn the ledger refuses to fund one, and the attempt declines.
  // That is the fix working, not the fix missing: before it, the compaction
  // went around the ledger entirely.
  assert.equal(summarizerCalls, 0);
  assert.deepEqual(nested.compactions, []);
...
test("M5: the subagent cost ledger is live and its refusal reaches the caller", async () => {
  // The ledger path itself, asserted end to end: a child call that settles
  // above what its wallet's ledger reserved raises cave_subagent_cost_budget,
  // and the parent sees it. This is the mechanism a compaction now shares.
  const child = agent({
    id: "m5-ledger-child",
    instructions: "Answer.",
    model: "anthropic/claude-haiku-4-5",
...
test("N2: a gateway-routed run is judged on the account credential", async () => {
  // On the gateway the provider key is the account's, not the local login's.
