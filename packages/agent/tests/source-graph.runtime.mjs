...
test("source graph resolves import-only package exports used by generated config", async (t) => {
  ...
  await mkdir(resolve(root, "node_modules/@caveman/agent/dist"), { recursive: true });
  await mkdir(resolve(root, "node_modules/runtime-leaf"), { recursive: true });
  await writeFile(resolve(root, "caveman.config.mjs"), [
    'import { defineBuild } from "@caveman/agent/build";',
    'export default defineBuild({ entry: "src/agent.ts" });',
  ].join("\n"));
  await writeFile(resolve(root, "node_modules/@caveman/agent/package.json"), JSON.stringify({
    name: "@caveman/agent",
    type: "module",
    exports: {
  ...
  await writeFile(
    resolve(root, "node_modules/@caveman/agent/dist/build.js"),
    'export { defineBuild } from "./runtime.js";',
  );
  await writeFile(
    resolve(root, "node_modules/@caveman/agent/dist/runtime.js"),
  ...
  assert.equal(
    manifest.some((entry) => entry.name === "node_modules/@caveman/agent/dist/build.js"),
    true,
  );
  assert.equal(
    manifest.some((entry) => entry.name === "node_modules/@caveman/agent/dist/runtime.js"),
    true,
  );
  assert.equal(
...
test("source graph resolves dependency closure from pnpm package symlinks", async (t) => {
  ...
  const agentStore = resolve(storeRoot, "@caveman+agent@0.1.0/node_modules");
  const agentRoot = resolve(agentStore, "@caveman/agent");
  const leafStore = resolve(storeRoot, "runtime-leaf@1.0.0/node_modules/runtime-leaf");
  await mkdir(resolve(root, "node_modules/@caveman"), { recursive: true });
  ...
  await symlink(
    resolve(storeRoot, "@caveman+agent@0.1.0/node_modules/@caveman/agent"),
    resolve(root, "node_modules/@caveman/agent"),
    "dir",
  );
  await symlink(
