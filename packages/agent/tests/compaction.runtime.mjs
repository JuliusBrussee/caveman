 ...
 * rates. On the run's own model that ceiling is essentially the working call's,
 * and the trigger is the moment that working call stops fitting — so the rung
 * only clears its precondition when the summarizer is genuinely cheaper. These
 * runs use an Opus-class working model with a Haiku-class summarizer, which is
 * the shape the cheap-class opt-in exists for.
 */
