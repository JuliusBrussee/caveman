...
def test_bundle_counters_accept_any_finite_json_number() -> None:
    """JSON has one number type: a publisher may emit 7 or 7.0 for the same
    counter and the TypeScript mirror accepts both. Booleans are not counters,
    and an integer too wide for a float is not one either."""
    ...
    rejected = _client_holding(_bundle_with_policies([], policy_version=True, sequence=10**400)).state()
