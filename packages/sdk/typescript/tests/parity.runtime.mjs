...
const handlers = {
  ...
  artifacts_page: (cave, input) =>
    cave.trace({}, async (t) => t.artifacts.page(input.value, input.options)),
  model_create_async: (cave, input) =>
    cave.trace({}, async (t) => t.model.openai.responses.create(input.body, { cave: { latencyClass: input.latency_class } })),
  bedrock: (cave, input) => {
  ...
  checkpoint: (cave, input) => cave.trace({}, async (t) => t.context.checkpoint(input.messages, input.options)),
  checkpoint_expand: (cave, input) => cave.trace({}, async (t) => t.context.expand(input.source_ref)),
  event_tool_call: (cave, input) => cave.trace({ tags: input.tags }, async (t) => t.tool(input.name, input.options, () => "tool-result")),
