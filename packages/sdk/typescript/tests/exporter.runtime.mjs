...
// Import compiled dist (must run `pnpm --filter @caveman/sdk build` first)
...
test("recordSpan() maps GenAI fields to gen_ai.* attributes and generates ids", () => {
  ...
  assert.equal(a["gen_ai.usage.cached_tokens"], 800);
...
test("recordSpan() omits malformed counters and prevents reserved-attribute overrides", () => {
  ...
  assert.ok(!("gen_ai.usage.cached_tokens" in span.attributes));
  assert.ok(!("gen_ai.usage.cost_usd" in span.attributes));
  assert.equal(span.attributes["cave.agent"], "real-agent");
});

test("export() POSTs OTLP payload to /otlp/v1/traces with Caveman headers", async () => {
  ...
  try {
    ...
    // Endpoint + method.
    assert.equal(req.url, "http://localhost:8787/otlp/v1/traces");
    ...
    assert.equal(chatAttrs["gen_ai.system"], "openai");
    ...
    assert.equal(chatAttrs["gen_ai.usage.cached_tokens"], "800");
