...
export class CaveTrace {
  ...
  async tool<T>(name: string, options: ToolOptions, fn: () => Promise<T> | T): Promise<T> {
    ...
    try {
      ...
      await request(this.cave, "/sdk/v1/events", { span_type: "tool.call", name, workflow: this.workflow, options, duration_ms: Date.now() - start, tags: this.tags }, this.workflow, this.traceContext()).catch(() => undefined);
    }
  }
...
export class RuntimePolicyClient {
  ...
  async refresh(): Promise<RuntimePolicyRefresh> {
    ...
    try {
      ...
      const next = finiteNumber(parsed["sequence"]);
      const current = this.bundle === null ? null : finiteNumber(this.bundle["sequence"]);
  ...
  private async verifyBundle(
    ...
    if (signatureRecord !== null || keyRecord !== null) {
...
async function caveFetch(cave: Cave, input: RequestInfo | URL, init: RequestInit = {}): Promise<Response> {
  const timeout = AbortSignal.timeout(cave.options.timeoutMs ?? DEFAULT_HTTP_TIMEOUT_MS);
  const signals = [timeout, cave.options.signal, init.signal].filter((signal): signal is AbortSignal => signal !== undefined && signal !== null);
  ...
  return Promise.race([fetch(input, { ...init, signal }), abort]);
}
...
function providerRawFetch(cave: Cave, prefix: string, input: RequestInfo | URL, init?: RequestInit, upstreamKey?: string): Promise<Response> {
  ...
  const rawInput = input instanceof Request ? input.url : String(input);
  const target = new URL(rawInput, base);
  ...
  for (const [name, value] of Object.entries(gatewayHeaders)) merged.set(name, value);
  return caveFetch(cave, target, { ...init, headers: merged });
}
