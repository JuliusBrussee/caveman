...
- `src/middleware/` — the experimental `@caveman-ai/sdk/middleware` subpath: `runtime.ts` (protocol client), `protocol.ts` (pure protocol 1.1 rules), `validate.ts` (capabilities/plan/page validation), `transport.ts` (lazy `node:http` transport used when HTTP(S)_PROXY applies, `ca` is set or Node runs with NODE_USE_ENV_PROXY; never proxies loopback). `tests/middleware-v1_1.runtime.mjs` drives every client vector in `../../parity/middleware-v1_1.fixtures.json` (the Python SDK runs the same file); `tests/middleware-hardening.runtime.mjs` pins the B3–B12 hardening fixes
